...
## 8. H100 での測定（TSUBAME、job `8502531` / `8502578`）
...
### 8.6 スクリプトの不具合（`8502531` の nsys 段）
...
- **x GEMM で 16x8x8 が負ける理由の確定。** `ncu` でスレッドあたりレジスタ数、
  spill、achieved occupancy、stall 内訳を 8x8x4 / 16x8x4 / 16x8x8 で比べる。
  レジスタ圧が主因なら、x だけタイルを 64x64x16 に落として 16x8x8 を使う、
  あるいは x だけ 16x8x4 を使う（GEMM ごとに形状を選べるようにする）といった
  手が見えてくる。ただし現状は **16x8x4 が 3 本とも最良か同着**なので、
  GEMM ごとの選択を足す動機はいまのところ無い。
- **既定値をどうするか。** H100 で 16x8x4 が明確に速く、GB200 で同着である以上、
  既定を `16x8x4` にする案がある。ただし `arch::Sm90` タグが既定になるので、
  sm_80 以前で組む可能性を考えるならビルド時の分岐が要る。
- **残る 1.27 倍。** 命令形状では埋まらない。同一タイル・同一命令でも
  cuBLAS の sm90 カーネルの方が速い（§8.5）ので、次に効くとすれば
