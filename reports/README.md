...
| [`p7_gemm_fused.md`](p7_gemm_fused.md) | p=7 `CUDAFORTRAN_GEMM_FUSED` の役割準拠 CUTLASS tile を調整し **2532.9 → 1960.3 µs/stage（−22.6%）**。y 32x64 s3、z 16x32 s3。旧 380 µs の要素 CTA は `FUSED` 相当なので範囲外として訂正。**§11（2026-09-01）は batch > 65535 の chunk launch を 1 ローンチにして 1960.3 → 1895.3 µs/stage（−3.27%、job `74632`）**。**§12 は y epilogue の repad で 1896.1 → 1818.1 µs/stage（−4.11%、job `74636`）** |
