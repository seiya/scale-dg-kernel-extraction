...
| [`p127_gap_study.md`](p127_gap_study.md) | 同一 DOF の 5 点目 p=127 (Nq=128)。**演算強度がマシンバランスを越える最初の点**。コード変更ゼロで 5 経路が通り、次数依存ノブ 4 種を掃引しても採用すべき変更が無かったこと、`K` が深くなると CUTLASS の x GEMM が cuBLAS に追いつくこと |
...
- **p=127, `Ne=2³`（2026-08-27）**: 同一 DOF の 5 点目で、**演算強度がマシンバランスを
  越える最初の点**。最速は **`CUDAFORTRAN_GEMM_FUSED`**、Main **2.3941 ms/step** /
  **710.3 µs/stage**（graph on 2.3383、−2.3%）。`CUDAFORTRAN_GEMM` の 1.16 倍、
  ...
  演算強度は **5.34 FLOP/B** で**マシンバランス 5.06（40.1 TFLOP/s ÷ 7.92 TB/s）の
