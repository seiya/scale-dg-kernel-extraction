...
| 255 | 918.9 | **883.8** | `p255_gap_study.md` §22、job `73986` | **1525.0**（同 §26.7、job `78165`） | **1.73×** | 2.04× |
...
**残るのはブロック形状 `(16,8)` の 1 軸だけ**で、天井は §27.4 で現行形に
引き直してある（Q≡1 −14.7% / D≡1 −11.6% / バリア全削除 −3.7% /
INNER=1 −35.0%、job `78273`）。**最速経路の順位はどの次数でも変わらない**
...
p=255 は 2026-09-03 に分母が 1868.6 → 1525.0 に動いたので **1.73×**（旧 2.11×）。
