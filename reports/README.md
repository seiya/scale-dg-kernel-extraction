...
| [`tc_paper_survey_2407.09621.md`](tc_paper_survey_2407.09621.md) | arXiv:2407.09621 の取り込み調査と、p=7 Tensor Core カーネルの shared memory レイアウト刷新。経路横断（§15）と CC 復活（§17）。CC のその後は [`p7_gap_study.md`](p7_gap_study.md)。§14 は p=31 で効いた「D1D フラグメントのレジスタ常駐化」が p=7 では**効かない**ことの実測 **§18（2026-09-02）は `FUSED_DFMA` の現行ソース測り直し（424.31 µs/stage、機構比 A 1.551×）。占有 GPU では §17 の 424.1 が再現し、§15 の 427.8 との 0.87% 差は login のばらつきだったことが確定** |
