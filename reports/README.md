...
- **p=63, `Ne=4³`（2026-08-28、`p63_gap_study.md` §16）**: **最速は `CUDAFORTRAN_FUSED_TC`**
  （**539.0** 対 `GEMM_FUSED` 587.3 µs/stage、**1.090 倍**）。前日の 662.3 µs から **−18.5%**。
  効いたのは 2 つだけで、どちらもレイテンシに効く変更である。
  ...
  **未解明**: y カーネルの `sFV` だけは転置すると wavefront も L1/TEX も改善するのに 22% 遅い（§16.6）。
