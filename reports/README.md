...
| [`p31_gap_study.md`](p31_gap_study.md) | 同一 DOF の 6 点目にして最後の点 p=31 (Nq=32)。**曲線の極大点**で、p=15 と p=63 の両方より遅く、5 点で見えていたピーク比の単調性が崩れる。融合が勝つ次数の上限は p=15 のままだと確定した（交差は p=15 と p=31 の間）。Nq=32 の融合カーネル 2 本を書いて GEMM に 1.38 倍負けた理由（融合の転送量優位が 1.13 倍まで縮み、L1/TEX で詰まる）。CUTLASS 経路は p=31 で使えるという訂正。**lift と assembly の融合で `GEMM` / `GEMM_CUTE` 経路を全次数 1 割速く**した（p=31 −11.7%、p=63 −12.0%、p=127 −10.2%、ビット一致） |
...
- **p=127, `Ne=2³`（2026-08-27）**: 同一 DOF の 5 点目で、**演算強度がマシンバランスを
  ...
  **（追記 2026-08-27）この単調性は 6 点目を入れると崩れる。** p=31 は 12.5% で 6 点中最小で、
...
- **p=31, `Ne=8³`（2026-08-27）**: 同一 DOF の 6 点目にして最後の点。**最速は素の
  ...
  **p=31 は曲線の極大点である**: 同一ビルド・同一 `nstep` で 434.8（p=15、CUDA core
