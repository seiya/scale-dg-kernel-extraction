...
| [`p31_gap_study.md`](p31_gap_study.md) | 同一 DOF の 6 点目にして最後の点 p=31 (Nq=32)。**曲線の極大点**で、p=15 と p=63 の両方より遅く、5 点で見えていたピーク比の単調性が崩れる。融合が勝つ次数の上限は p=15 のままだと確定した（交差は p=15 と p=31 の間）。Nq=32 の融合カーネル 2 本を書いて GEMM に 1.38 倍負けた理由（融合の転送量優位が 1.13 倍まで縮み、L1/TEX で詰まる）。CUTLASS 経路は p=31 で使えるという訂正 |
