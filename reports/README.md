...
**§28（同日）は `Nq>64` の z epilogue 4 つを p=31 に開こうとして採用ゼロ**：16 B lift +0.94%（job `75869`、12/12、レンジ非重複）、クランプ +2.8%、16 B epilogue +13.9%、3 つ揃えると **+69.1%**。ncu job `75870` は命令が −5.9% なのに時間 2.17 倍で、レジスタ 254 の z が `double2` の lift ステージングを置けず 16.3 M 回スピルする（p=7 は 128 → 164 でスピル 0）。「命令発行律速だから命令を減らせば速い」はレジスタ予算に余裕のある次数でだけ成り立つ |
...
|---:|---:|---:|---|---:|---:|---:|
| 7 | 274.9 | 274.9 | 変化なし | 302.8（`p7_gap_study.md`、job `70538`） | **1.10×** | 1.10× |
...
| 63 | 421.7 | **409.2** | `p63_gap_study.md` §50.6、job `75806` | 618.6（同 §25、job `69651`） | **1.51×** | 1.47× |
| 127 | 621.5 | **590.8** | `p127_gap_study.md` §23.8、job `75895` | 1149.9（同 §17、job `70527`） | **1.95×** | 1.85× |
| 255 | 918.9 | **883.8** | `p255_gap_study.md` §22、job `73986` | 1868.6（同 §15.92） | **2.11×** | 2.04× |

**4 次数（31 / 63 / 127 / 255）で動いたのは分子（TC）だけで、分母（CC）は
上表の行と同じ値である**（`FUSED` 側の横展開は `TODO.md` §2.2 で未試行）。
p=63 の **1.51×** と p=127 の **1.95×** はどのレポートにも書かれていなかった値で、
ここが初出。p=255 の 2.11× と p=31 の 1.42× は `p31_gap_study.md` §25.2 および
本ファイルの次数別節に一致する。**最速経路の順位はどの次数でも変わらない。**
