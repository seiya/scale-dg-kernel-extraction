...
**この軸を当てていないのは p=255 CC だけ**（`TODO.md` §6 の「`p255_gap_study.md`
に `FUSED` の探索終了宣言が無い」に対応する）。**最速経路の順位はどの次数でも変わらない**
（6 次数とも `FUSED_TC`。ただし p=63 では CC 融合が `GEMM_FUSED` を初めて
17.4% 下回った）。

p=255 の 2.11× と p=31 の 1.42× は `p31_gap_study.md` §25.2 および
本ファイルの次数別節に一致する。
