...
p=15 §18–19、p=31 §16–20、p=63 §22–23、p=127 §15–17、p=255 §12–15.11。
...
| `CUDAFORTRAN_FUSED` | 1.280 | 344.6 | 6.03 | 15.0% | 3.85 | 48.7% | 5.01 | 63.4% |
...
`CUDAFORTRAN_FUSED` は [`p15_gap_study.md`](p15_gap_study.md) §21 の CC 融合
（device **344.6 µs**、job `70533`）。§19 の 446.7 µs は改修前。論文の主比は
**TC / FUSED = 271.8 / 344.6 = 1.27×**。§20 は `FUSED_TC` の探索終了。
...
論文の主比は **TC / FUSED = 706.1 / 1149.9 = 1.61×**。
点変化係数の owned `dqdt` は性能と同じ `Ne=2³`（16,777,216 点）で SPLIT に対し
max abs 3.55e-15（[`p127_gap_study.md`](p127_gap_study.md) §17）。
...
p=31 / p=7 / p=127 の `FUSED` 行**で、[`p31_gap_study.md`](p31_gap_study.md) §20
（job `69623`、718.2 µs）、[`p7_gap_study.md`](p7_gap_study.md)（job `70538`、
302.8 µs）、[`p127_gap_study.md`](p127_gap_study.md) §17（job `70527`、
...
- **p=15 `CUDAFORTRAN_FUSED`（2026-08-30、`p15_gap_study.md` §21）**: CUDA-core
  融合を **445.7 → 344.6 µs/stage（−22.7%、ビット一致）**にした。job `70533`
  （c384）12 回交互 A/B。効いたのは `__restrict__`（−10.5%）、面 gather の
  x パネル直後への前倒し（−8.2%）、512 スレッド 2 CTA/SM（−5.2%）。最速は
  `FUSED_TC`（271.8 µs）のまま。論文の主比は 1.64× → **1.27×**。内積を消した
  天井は −31%、面 gather を消した天井は −24%。
