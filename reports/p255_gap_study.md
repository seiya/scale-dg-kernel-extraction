...
## 26. レジスタ予算の壁を測ってから `TODO.md` §3 の 3 件を片付ける（2026-09-02〜03）

`TODO.md` §3 が p=255 に残していた 3 行

- `FUSED_TC`: `BK=32`（バリア半減）＝ §7「未実装」
- `FUSED_TC`: x と y の融合＝ §7・§9「未実装」
- `FUSED`（CC）: z の D の global 化＝ §15「z は y と同型のため未実装」

は、前 2 つが「**レジスタ予算が理由で止まっている**」と書かれていた。そこで
まず予算の壁が実測でどこにあるかを ptxas で出し、そのうえで 1 件ずつ測った。
**3 件とも不採用**。一方、併せて点検した「出力タイル (A) × レジスタ予算 (B)」
の**次の段**が p=255 CC では未適用で、これは **−18.6% で採用**になった。

### 26.1 測定条件と物差し

- 親 commit `5fb1b8a`（`feature/cuda`）＋本節の作業ツリー、GB200 1 GPU、
  `make CUDA=1 GPUFLAGS=-gpu=cc100`
- 入力 `namelists/perf_p255_fused_tc.conf` / `namelists/perf_p255_fused.conf`
  （どちらも `Ne=1`、`PolyOrder=255`、`nstep=20`、`UseCudaGraph=.false.`。
  `NeX/NeY/NeZ`・`dt`・`nstep` は全比較で固定）
- **物差しは `CUDA device fused tendency` ÷ (19 steps × 3 RK) = ÷57**
  （`p63_gap_study.md` §52.5 の 3 種類のうちの 1 つ。`Step loop per stage` や
  `Cal_tend` と同じ表に並べない）
- 採否の A/B は **Slurm で占有した GPU 上で全変種を交互に 12 ラウンド**
  （job **`78165`**、node **c186**）。追加の天井測定は job **`78175`**、node c186。
  ncu は job **`78172`**、node c186、`--set` ではなくメトリクス指定
  （`jobs/ncu_p255_regbudget.sh`。conf は計算ノードから見える
  `jobs/perf_p255_fused{,_tc}_ncu.conf`、`nstep=4`）。
  **ncu の時間は採否に使っていない**（クロック固定バイアス）。
- ptxas は `nvcc -O3 -std=c++17 -arch=sm_100 -Ithird_party/cutlass/include
  --expt-relaxed-constexpr -Xptxas -v`

同一ジョブのベースライン（12 ラウンド中央値、`µs/stage`）:

| 経路 | 中央値 | レンジ | §25（job `78053`）の値 |
|---|---:|---:|---:|
| `CUDAFORTRAN_FUSED_TC` | **883.55** | 882.75–883.96 | 885.30 |
| `CUDAFORTRAN_FUSED` | **1874.26** | 1873.11–1875.70 | 1875.23 |

§25 とは 0.2% / 0.05% で一致する。以下の % はすべてこの同一ジョブ内の中央値比。

### 26.2 レジスタ予算の壁（ptxas 実測）

`tendency_p255_kernel`（TC、128 スレッド）:

| `BK255` / `MINB255` | レジスタ | spill | shared | 予算の由来 |
|---|---:|---:|---|---|
| **16 / 3（本番）** | **168** | **0** | 32 KB 静的 | 65536/(128·3) = 170.7 |
| 16 / 2 | 208 | 0 | 32 KB 静的 | 65536/(128·2) = 256 |
| 32 / 2 | **255** | **0** | 64 KB 動的 | 上限 255 に張り付く |
| 32 / 3 | 168 | 100 B store / 52 B load | 64 KB 動的 | 168 に押し込まれて溢れる |

**壁は 255 レジスタで、`BK=32` はそこにちょうど収まる。** §7 が書いていた
「プリフェッチが 48 → 96 になり `MINB=3` に入らない」は正しく、`MINB=2` まで
落とせば spill 0 で入る（`BK=32` は静的 48 KB も超えるので、`P255_DYNSMEM`
で動的 shared に切り替える実装を入れた。`BK=16` の既定は静的のまま）。

副産物として、**本番の `BK=16` でも `FUSED_DFMA` 側（`UseTc=false`）は
168 レジスタで 56–124 B の spill を出している**（TC 側は 0）。同じソース・
同じ日程で、mma を DFMA に置き換えた結果 ptxas の割り当てが変わる。
§25 が「`FUSED_DFMA` だけ旧値より +0.95% 遅い」と書いて断定を保留した点の、
少なくとも一因の候補になる（本節では時間は測っていない）。

`tendency_[xyz]_p255_cc_kernel`（CC、128 スレッド）:

| カーネル | 出力/スレッド | `minBlocks` | レジスタ | spill | shared |
|---|---|---:|---:|---:|---|
| x（本番） | 2 i × 4 j | 8 | 64 | 0 | 8448 B |
| y（本番） | 2 i × 4 j | 8 | 64 | 0 | 16384 B |
| z（本番） | 2 line × 4 k | 8 | 64 | 0 | 8192 B |

3 本とも 65536/(128·8) = 64 の天井に張り付いている（§23.2 の再確認）。

### 26.3 (1) `FUSED_TC` の `BK=32` —— 不採用、**バリアは律速ではない**

`sw255` は `l + 16*outer` を前提に shift 4 / 6 を焼き込んでいたので、
`l + BK*outer` の一般形（shift `log2(BK)` / `log2(BK)+2`）に直し、
`abase` / `bbase` / staging の `16*o` も `BK*o` にした。`BK=16` では
生成コードが変わらず、点変化係数の owned `dqdt`（`Ne=1`、16,777,216 点）は
改修前と**ビット一致**。`BK=32` の 2 形もベースラインと**ビット一致**である
（チャンクを切る位置が変わっても l の縮約順序は同じ）。

| 候補 | 中央値 µs/stage | レンジ | 対ベース | 採否 |
|---|---:|---:|---:|---|
| 現行（`BK=16`, `MINB=3`, 168 reg） | **883.55** | 882.75–883.96 | — | — |
| `MINB=2` だけ（208 reg、予算を買うだけ） | 970.07 | 969.21–970.49 | **+9.79%** | 予算の値段 |
| **`BK=32`, `MINB=2`**（255 reg, 動的 64 KB） | 980.75 | 980.17–981.84 | **+11.00%** | **不採用** |
| `BK=32`, `MINB=3`（168 reg, 100 B spill） | 1160.57 | 1159.80–1162.01 | **+31.35%** | 不採用 |

全レンジが互いに重ならない。§16 の `BK=32` 単一バッファ +37%、§17 の
`BK=32` 二重バッファ `MINB=2` 「〜+11%」を、正しい swizzle の実装で
**追認**した形である。

**機構（ncu job `78172`、採否には使わない）。** 同じ `MINB=2` の上で
`BK=16` → `BK=32` を比べると、

| | barrier stall | inst | shared WF | global sector | 占有率 |
|---|---:|---:|---:|---:|---:|
| `MINB=2`, `BK=16` | 1133 | 9.345e7 | 2.552e7 | 5.80e7 | 12.25% |
| `MINB=2`, `BK=32` | 1074 | 9.601e7 | 2.545e7 | 5.80e7 | 12.23% |

**チャンク数を 16 → 8 にしてもバリア stall は 5% しか減らず、命令は 2.7% 増える。**
本番形（`MINB=3`）でもバリア stall は 3370 に対し `math_pipe` 21450 /
`wait` 17870 で、**バリアはもともと律速ではない**。したがって
「占有率を下げて予算を作る」（`p127_gap_study.md` §23.2 / §23.9）は
ここでは**成り立たない**: 予算の代金が **+9.79%** なのに、買ったもの
（`BK=32`）が返すのは **−0%**（iso 占有率で見ると +1.10%）である。
p=127 xz で同じ解法が効いたのは、あちらが `long_scoreboard` 11.37 の
global 待ちで、予算で買った二重バッファに**隠す相手があった**からで、
p=255 TC の `math_pipe` + `wait` にはその相手がいない（§6・§22 と同じ結論）。

### 26.4 (2) `FUSED_TC` の x + y 融合 —— 不採用、天井 **14.9 µs** に対し予算の代金 86 µs

x と y は同じ出力タイル `C^T[j][i]` を持つので、融合すれば **x の `dqdt`
ストア 16 MB と y の `dqdt` 読み戻し 16 MB** が消える。それが賞金の全部で
あり、レジスタは §7 のとおり蓄積器 2 組（128）＋ プリフェッチ 4 パネル（96）
＝ 224 以上を要求するので、26.2 の壁から **`MINB=2` でしか実装できない**。

賞金を実装せずに測るため、不正アブレーションを 3 形入れた（job `78175`、
c186、12 ラウンド交互。いずれも `dqdt` は壊れる。天井専用）:

| アブレーション | 中央値 µs/stage | レンジ | 対ベース | µs |
|---|---:|---:|---:|---:|
| 現行 | 883.52 | 883.00–884.10 | — | — |
| x の `dqdt` ストアだけ消す（**tight**） | 870.52 | 869.99–870.98 | **−1.47%** | −13.0 |
| y の `dqdt` 読みだけ消す（**tight**） | 881.79 | 881.33–882.10 | **−0.20%** | −1.7 |
| **両方（＝融合の賞金の全部）** | **868.66** | 868.36–868.88 | **−1.68%** | **−14.9** |
| （参考）ストアを never-taken 分岐に置いた **loose** 形 | 714.84 | 713.87–715.19 | −19.09% | −168.7 |

**loose 形は天井ではない。** ストアを `if (npoint < 0)` の下に置くと ptxas が
x エピローグ全体をその分岐に沈め（DIR=0 が 168 → **110 レジスタ**）、
融合が残すはずの `Escale` / lift / face のロードと flop まで消える。
tight 形は結果を 1 個のスカラーに畳んで「生きているが決して実行されない」
ストア 1 本に落とすので 168 レジスタが保たれ、ロードも flop も全部残る。
**この 12.8 倍の差そのものが、この種のアブレーションの書き方に関する記録である。**

判定は数え上げで閉じる: 賞金 **14.9 µs**（−1.68%）に対し、実装可能な唯一の
予算 `MINB=2` の代金は 26.3 で測ったとおり **+86.5 µs**（+9.79%）。
差は 5.8 倍で、レンジの幅（0.5 µs 級）に比べて桁が違う。**不採用**。
y の読み側が −0.20% しかないのは §20 と整合する（あちらは y+z 合わせて
−0.66%）。「占有率を下げて予算を作る」はここでも成り立たない。

### 26.5 (3) `FUSED`（CC）の z の D の global 化 —— 不採用 **+11.26%**

§15.8 は x の D を shared から `__ldg` に移して勝ち、§15.86 は同じことを y で
やって負けた。z は y と同型なので未実装のまま残っていた。実装して測った。

| 候補 | 中央値 µs/stage | レンジ | 対ベース | 採否 |
|---|---:|---:|---:|---|
| 現行（z の D は shared `sD`） | **1874.26** | 1873.11–1875.70 | — | — |
| z の D を `__ldg`（`sD` 撤去、shared 8192 → 4096 B） | 2085.30 | 2079.02–2090.44 | **+11.26%** | **不採用** |

レンジ非重複。点変化係数の owned `dqdt` は `Ne=1` で改修前と**ビット一致**。

**機構（ncu job `78172`、z カーネル）。**

| | shared WF | global request | global sector | `long_scoreboard` | `lg_throttle` |
|---|---:|---:|---:|---:|---:|
| 現行 | 1.521e8 | 1.494e7 | 1.111e8 | 1.908e4 | 419 |
| z の D を `__ldg` | 7.673e7 (**−50%**) | 7.786e7 (**+421%**) | 1.447e8 (+30%) | 5.253e4 (**+175%**) | 4973 (**×11.9**) |

狙った shared の半減は当たるが、**global リクエストが 5.2 倍になる**。
理由は x と z で D の添字が乗っているスレッド軸が違うことに尽きる:

- x は `i0 = pair_i*32 + tx` で、**速い方の添字 `tx`** に乗っている。
  半ワープ 16 レーンが D1D の連続 16 要素を読むので 1 リクエストで済む。
- z は `k0 = quad_k*32 + ty` で、**遅い方の添字 `ty`** に乗っている。
  ワープ内で k は 2 値しか無く、同じ値を 16 レーンが読むうえ、
  内積の t ごとに読み直すので、平面あたり 1 回だった shared 充填が
  16 回の global リクエストになる。

y も同じ形なので §15.86 の負けと同じ機構であり、**z が y と同型だという
§15 の見立ては正しかった**（ただし「だから測らなくてよい」ではなかった）。

### 26.6 出力タイル (A) × レジスタ予算 (B) の**次の段** —— CC で **−18.6%、採用**

`TODO.md` は「(A)×(B) の横展開が p=255 CC に適用済みか」を点検事項として
残していた。答えは**「1 段目は適用済み、2 段目は未適用」**である。

- 1 段目は §15.68–§15.76 そのもので、**この軸はここが発祥**である
  （1 スレッド 8 出力 ＋ `minBlocks=8` ＝ 64 レジスタ）。
- ところが p=63 / p=127 が `TODO.md` §2.2 で到達したのは
  **128 レジスタ・占有率 24%** であり、p=255 CC は **64 レジスタ・占有率 50%**
  のままだった。同じ梯子の 2 段目（出力 8 → 16、32）を誰も登っていない。

そこで x / y / z のそれぞれについて「i（あるいは line）方向の出力数 `TI`」と
`minBlocks` を独立に振った。login node 1 回走らせた掃引（他 2 本は既定のまま。
差は 1% 以上あるので掃引にはこれで足りる。**採否は 26.7 の占有 GPU A/B**）:

| カーネル | `TI` | `minBlocks` | レジスタ | spill | login µs/stage |
|---|---:|---:|---:|---:|---:|
| （既定 3 本とも） | 2 | 8 | 64 | 0 | 1878.3 |
| x | 4 | 8 | 64 | 0 | 2222.7 |
| x | 4 | 6 | 80 | 0 | 1801.2 |
| x | 4 | 5 | 96 | 0 | 1757.5 |
| x | 4 | 4 | 128 | 0 | 1745.6 |
...
**(B) の符号則は p=255 でも両側に出る**: x の 4 出力/i は予算 128 レジスタなら
**−7.0%**、同じタイルを 64 レジスタのまま要求すると **+18.4%**
（spill は 0 だが、要求した占有率を保つために ptxas が再実体化する）。
z の 4 出力は 64 レジスタで 8 B の spill を出す。`p7_gap_study.md` §5 が
書いた「(B) は『spill させない』だけでなく『達成可能な占有率を要求する』でもある」
の 3 例目である。y だけは (B) の効きが弱く（`TI=4` では 64 と 108 で差が無い）、
`TI=8` にして初めて 168 レジスタが要る。

y は `TI=16` で静的 shared 48 KB を超えるので測れない。z の `TI=16` は
254 レジスタ・spill 0 で入るが **+4.7%**（3 本の 4 平面 `sQ` が 32 KB を占め、
CTA が 1 枚しか載らない）。x の `TI=16` も同様に負ける。**梯子はここで終わる。**

### 26.7 採用形と占有 GPU A/B（job `78165`、c186）

採用は **x: 8 i × 4 j / `minBlocks=3`（168 reg）、y: 8 i × 4 j /
`minBlocks=2`（168 reg）、z: 8 line × 4 k / `minBlocks=2`（166 reg）**。
`fused_kernel_geom.h` ではなく `cuda_dg_kernels_fused_highp.cu` の
`P255_CC_{X,Y}_TI` / `P255_CC_Z_TL` / `P255_CC_{X,Y,Z}_MINB` をノブとして残した
（TC と共有しないため。`P255_CC_ZDG` は 26.5 の不採用形）。

| 候補 | 中央値 µs/stage | レンジ | 対ベース |
|---|---:|---:|---:|
| 現行 CC | 1874.26 | 1873.11–1875.70 | — |
| x だけ 8 i / `MINB=3` | 1700.89 | 1698.97–1701.34 | **−9.25%** |
| y だけ 8 i / `MINB=2` | 1763.32 | 1761.32–1765.04 | **−5.92%** |
| z だけ 8 line / `MINB=2` | 1826.27 | 1824.63–1828.33 | **−2.56%** |
| z だけ 4 line / `MINB=4` | 1841.52 | 1840.98–1843.95 | −1.75% |
| **3 本とも（採用）** | **1525.00** | 1521.34–1530.21 | **−18.63%** |

全レンジ非重複。別ジョブ（`78173`、同 c186、12 ラウンド）でも
1874.61 → 1523.94（**−18.71%**）で再現した。**3 本の効果はほぼ加法的**
（−9.25 −5.92 −2.56 = −17.7% に対し実測 −18.6%）で、カーネルが直列に走る
以上これは期待どおりである。

**機構（ncu job `78172`）。**

| カーネル | | shared WF | inst | `mio_throttle` | L1/TEX % | global sector | DRAM read | 占有率 |
|---|---|---:|---:|---:|---:|---:|---:|---:|
| x | 現行 | 7.607e7 | 2.958e8 | 1.732e4 | 92.95 | 2.071e8 | 4.043e8 | 48.77% |
| x | 採用 | 1.923e7 (**−74.7%**) | 1.993e8 (−32.6%) | 310 (**−98%**) | **79.24** | 1.564e8 (−24.5%) | 4.044e8 (**±0**) | 18.09% |
| y | 現行 | 1.537e8 | 3.137e8 | 2.507e4 | 97.02 | 1.106e8 | 5.387e8 | 48.70% |
| y | 採用 | 9.478e7 (−38.3%) | 2.122e8 (−32.4%) | 5147 (−79%) | **86.31** | 8.507e7 (−23.1%) | 5.385e8 (**±0**) | 18.01% |
| z | 現行 | 1.521e8 | 2.630e8 | 3.393e4 | 96.35 | 1.111e8 | 2.425e9 | 48.61% |
| z | 採用 | 9.475e7 (−37.7%) | 2.073e8 (−21.2%) | 5140 (−85%) | **86.12** | 8.598e7 (−22.6%) | 2.425e9 (**±0**) | 18.08% |

p=63 §53–§54 / p=127 §24–§25 と**同じ署名**である: shared wavefront と命令が
出力数で割られ、`mio_throttle` が実質消え、**L1/TEX が 93–97% の屋根から
79–86% に降り**、**DRAM バイトは 1 バイトも動かない**。占有率は 48.7% →
18.1% に落ちるのに 18.6% 速い。global セクタが 22–25% 減るのは、
`Escale` / `dqdt` / `flux_bnd` のエピローグアクセスが 1 スレッドあたり
2 倍の連続範囲を覆うようになるためで、リクエスト数も x −15.4% / y −24.6% /
z −21.0% と同方向に動く。
...
| 比較 | 点数 | 結果 |
|---|---:|---|
| 採用後 CC 対 改修前 CC（`Ne=1`） | 16,777,216 | **全点ビット一致** |
| 採用後 CC 対 改修前 CC（`Ne=2³`） | 134,217,728 | **全点ビット一致** |
| 採用後 CC 対 `CUDAFORTRAN_GEMM`（`Ne=1`） | 16,777,216 | max abs **3.553e-15**、>1e-14 が 0 件 |
| 採用後 CC 対 `CUDAFORTRAN_GEMM`（`Ne=2³`） | 134,217,728 | max abs **3.553e-15**、>1e-14 が 0 件 |
| `FUSED_TC` 対 改修前（`sw255` 一般化後、`Ne=1`） | 16,777,216 | **全点ビット一致** |
| `FUSED_TC` 対 `FUSED_DFMA`（同ツリー、`Ne=1`） | 16,777,216 | **全点ビット一致** |
| `FUSED_TC` 対 `CUDAFORTRAN_GEMM`（`Ne=1`） | 16,777,216 | max abs **3.553e-15** |
| `BK=32` の 2 形 対 ベース（`Ne=1`） | 16,777,216 | **全点ビット一致** |
| z の D `__ldg` 対 ベース（`Ne=1`） | 16,777,216 | **全点ビット一致** |

p=255 に `CUDAFORTRAN_SPLIT` は無いので参照は `GEMM`（§25 と同じ）。
入力は `namelists/val_p255_fused.conf` / `val_p255_fused_tc.conf` /
`val_p255_gemm.conf` と、その `NeX/NeY/NeZ` だけを 2 にした作業コピー
（コミットしない）。`UseTc` に依存する分岐が `mma_m8n8k4_f64` の 1 か所だけで
あることは本節の変更後も変わらない（`sw255` の一般化も `P255_DYNSMEM` も
`UseTc` を見ない）。

### 26.9 到達点と、この節で動いた結論

- `CUDAFORTRAN_FUSED`（CC）: **1874.26 → 1525.00 µs/stage（−18.63%）**。
  26.1 の物差しで 26.11 GFLOP/stage なら **17.12 TFLOP/s = 40.1 の 42.7%**
  （改修前は 13.93 TFLOP/s、34.7%）。
- **主比 B（`FUSED` / `FUSED_TC`）は 2.118× → 1525.00 / 883.55 = 1.726×。**
- **最速経路は `CUDAFORTRAN_FUSED_TC` のまま**（883.55）。CC は
  `GEMM_FUSED`（§10 系列の 943–963 µs/stage）にもまだ届かない。
- `TODO.md` §3 の p=255 の 3 行はこれで閉じた。3 件とも不採用で、
  **2 件（`BK=32`、x+y 融合）は「レジスタ予算が壁」ではなく
  「予算を買っても買ったものが返さない」が正しい理由**だった。
  `p127_gap_study.md` §23.9 の「塞がっていたのは占有率であって性能では
  なかった」は p=127 xz では成り立ち、**p=255 TC では成り立たない**
  （`p31_gap_study.md` §30.5 に続く 2 例目の反例）。
- `TODO.md` §6 の「`p255_gap_study.md` に `FUSED` の探索終了宣言が無い」に
  対しては、**宣言を出さない**。26.6 の梯子は `TI=16` で終わったが、
  j 方向のタイル幅（3 本とも 4 で固定）とブロック形状 `(16,8)` は
  振っていない。ここは測れるものが残っている。

## 27. `TODO.md` §6 の再点検 —— `FUSED_TC` の宣言と `FUSED` の宣言（2026-09-03）

`TODO.md` §6 は「覆った 5 例はいずれも**当時の候補集合に、後で他次数から
来る軸が入っていなかった**」と書いて、残る宣言を再点検の対象に挙げていた。
p=255 に関わるのは 2 行である:

- **§22（`FUSED_TC`、2026-08-31、採用ゼロ）**。当時の候補集合に、
  §26.6 で CC が −18.6% を取った**出力タイル (A) × レジスタ予算 (B) の第 2 段**
  が入っていない。§26.6 は CC の話なので、TC 側は別に確認が要る。
- **`FUSED`（CC）には探索終了宣言が無い。** §26.9 が「j 方向のタイル幅と
  ブロック形状は振っていないので宣言を出さない」と明記して終わっている。

本節は前者を測って**維持**し、後者について**残っていた 2 軸のうち 1 軸
（j タイル幅）を測って**、天井を現行形で引き直す。

### 27.1 測定条件と物差し

- 親 commit `219e78d`（`feature/cuda`）＋本節の作業ツリー、GB200 1 GPU、
  `make CUDA=1 GPUFLAGS=-gpu=cc100`
- 入力 `namelists/perf_p255_fused_tc.conf` / `namelists/perf_p255_fused.conf`
  （`Ne=1`、`PolyOrder=255`、`nstep=20`、`UseCudaGraph=.false.` 固定）
- 物差しは **`CUDA device fused tendency` ÷ 57**（§26.1 と同じ）
- 採否・天井の A/B はすべて **Slurm で占有した GPU 上で 12 ラウンド交互**:
  TC は job **`78266`**（node **c384**）、CC の TJ は job **`78269`**（node **c177**）、
  CC の天井アブレーションは job **`78273`**（node **c177**）

### 27.2 `FUSED_TC` に出力タイル (A) の第 2 段を当てる —— **返らない。§22 を維持**

p=255 TC の 1 ブロックは 64×64 の出力タイルを 128 スレッド（2×2 ワープ格子、
1 ワープ 4×4 mma タイル）で持つ。**1 スレッドの出力は 32**（`acc[2*TM*TN]`）で、
CC の第 1 段（8 出力）よりすでに 4 倍大きい。(A) の「次の段」は
**スレッド数を 128 に据え置いたままタイルを 2 倍にする**ことで、
`BM255`/`BN255`/`TM255`/`TN255` を `#ifndef` 化して 4 形すべて測った
（§16 の 64×128 は **256 スレッド**なので出力/スレッドが変わっておらず、
(A) の軸ではない）。

ptxas（`-arch=sm_100`、`UseTc=true`、DIR ごと）:

| 形 | スレッド | 出力/スレッド | レジスタ | spill | shared |
|---|---:|---:|---:|---:|---|
| 64×64 / 4×4 / `MINB=3`（本番） | 128 | 32 | 168 | 0 | 32 KB 静的 |
| 64×64 / 4×4 / `MINB=2` | 128 | 32 | 208–254 | 0 | 32 KB |
| 64×128 / 4×8 | 128 | 64 | 254–255 | 0〜72 B | 48 KB（静的上限ちょうど） |
| 128×64 / 8×4 | 128 | 64 | 255 | 0〜40 B | 48 KB |
| 128×128 / 8×8 | 128 | 128 | 255 | 2.5 KB | **64 KB — 静的上限超過** |

**壁は 255 レジスタで、2 倍タイルはそこにちょうど収まる**（§26.2 が
`BK=32` について出したのと同じ形）。したがって実装可能な予算は `MINB=2` だけ
である。login 2-run の掃引（µs/stage）:

| 形 | login |
|---|---:|
| 本番（64×64, `MINB=3`） | 882.0–882.3 |
| 64×64, `MINB=2`（**予算だけ買う**） | 969.8 |
| 64×128 / 4×8, `MINB=2` | 987.1 |
| 64×128 / 8×4, `MINB=2` | 986.1 |
| 128×64 / 8×4, `MINB=2` | 983.3 |
| **128×64 / 4×8, `MINB=2`（4 形の最良）** | **972.3** |
| 64×128 / 4×8, `MINB=1` | 987.4 |
| 128×64 / 8×4, `MINB=1` | 982.7 |

最良形を占有 GPU で確認した（job `78266`、c384、12 ラウンド交互、
全レンジ非重複）:

| 変種 | 中央値 µs/stage | レンジ | 対本番 | 対 iso 予算 |
|---|---:|---:|---:|---:|
| 本番（64×64, `MINB=3`） | **885.03** | 883.77–886.45 | — | — |
| 64×64, `MINB=2`（予算の値段） | 971.49 | 971.12–972.40 | **+9.77%** | — |
| 128×64 / 4×8, `MINB=2`（(A) 第 2 段） | 974.65 | 974.24–975.21 | +10.13% | **+0.33%** |

**判定は §26.3 / §26.4 と同じ形で閉じる。** 予算 `MINB=2` の代金は
**+9.77%**（§26.3 の +9.79% を別ジョブで再現）、そして**その予算で買った
出力タイルが返すのは −0%**（iso 予算で見ると +0.33%、レンジ非重複で
むしろ遅い）。

**機構は §22 の ncu がすでに書いてある。** p=255 TC は L1/TEX **53–54%**、
shared バンクコンフリクトは wavefront の 0.5% 未満、支配 stall は
`math_pipe` + `wait` である。(A) が割る相手（shared wavefront・命令・バリア）
は**どれも屋根ではない**。p=63 / p=127 / p=255 CC / p=7 CC / p=7 TC で (A) が
効いたのは、いずれも L1/TEX が 91–98% の屋根にいた形であり
（`p15_gap_study.md` §27.3 の判定基準）、p=255 TC はその条件を満たさない。

**したがって §22 の「残る契約内の天井で測定誤差を超えるのは P 側本体の
~5 µs だけ」という宣言は、この軸も測ったうえで維持する。**
`p31_gap_study.md` §30.5・§26.3 に続いて、「占有率を下げてレジスタ予算を
作る」（`p127_gap_study.md` §23.9）が**成り立たない 3 例目**である。

### 27.3 `FUSED`（CC）の j タイル幅 —— 測った。**賞金は無い**

§26.9 が残した 2 軸のうちの 1 つ。x / y / z の 3 本とも「1 スレッドが持つ
j（z では k）の数」は **4 で固定**されていて、一度も振られていなかった。
x カーネル（3 本のうち単独効果が最大の −9.25%）について
`P255_CC_X_TJ` を入れて振った（shared 平面は j 2 本につき 1 枚、
ブロック分割は `nblock_pe = 2*BPE/(TI*TJ)`、`NQUAD = 256/(8*TJ)`）。

ptxas は全形 spill 0（`TI×TJ` = 8×2: 166 reg / 4224 B、8×4: 168 / 8448、
8×8 `MINB=2`: 254 / 16896、4×8: 168 / 16896、16×2: 168 / 4224）。
login 2-run の掃引（y/z は本番形に固定、µs/stage）:

| x の形 | login |
|---|---:|
| HEAD（`TI=8`, `TJ=4` の手書き形） | 1522.6–1529.2 |
| `TI=8`, `TJ=4`（**一般化コード、同じ形**） | 1536.6–1541.1 |
| `TI=8`, `TJ=2` | 1774.4–1775.4 |
| `TI=16`, `TJ=2` | 1779.2–1787.0 |
| `TI=2`, `TJ=8` | 1626.6–1630.5 |
| `TI=4`, `TJ=8` | 1520.2–1524.6 |
| `TI=8`, `TJ=8`, `MINB=1` | 1505.8–1530.8 |
| `TI=16`, `TJ=4`, `MINB=2` | 1680.4–1705.9 |

占有 GPU（job `78269`、c177、12 ラウンド交互）:

| 変種 | 中央値 µs/stage | レンジ | 対 HEAD |
|---|---:|---:|---:|
| HEAD（手書き `TJ=4`） | **1529.5** | 1526.1–1535.4 | — |
| 一般化コードの `TJ=4`（同じ形） | 1541.6 | 1538.1–1545.0 | **+0.79%** |
| `TI=4`, `TJ=8` | 1525.9 | 1523.6–1529.8 | −0.24%（**レンジ重複＝差なし**） |
| `TI=8`, `TJ=8`, `MINB=1` | 1529.1 | 1509.6–1557.9 | ±0（レンジ重複） |

**結論は 2 つある。**

1. **(A) の梯子は「出力数」で決まり、その形（i と j の配分）では決まらない。**
   `TI×TJ = 32` の 3 形（8×4、4×8、そして 16×2）のうち、8×4 と 4×8 は
   差が無く、**16×2 だけが +16%** である。16×2 が落ちるのは j が 2 本しか
   無いと内積の内側で shared から読む `f[a]` が 2 個になり、
   `TI+TJ` ロードで `TI*TJ` FMA を回すという比が悪化するからで、
   §26.6 の `TI=16`（+4.7%）と同じ理由である。逆に 64 出力（8×8）は
   32 出力と差が無く、**梯子は 32 出力で頭打ち**である。
2. **一般化そのものが 0.79% 遅い。** `TJ` をテンプレート化すると、
   `TJ=4` に畳んでも ptxas のレジスタ割り当てが変わり（レジスタ数・
   shared・spill はすべて同じなのに SASS が別物になる）、本番形が 0.79% 落ちる。
   `TJ=8` の最良形が取り返すのはちょうどその 0.79% で、**HEAD の手書き形は
   超えない**。

**採用ゼロ。ノブは残さずコードを HEAD に戻した**（§26.5 の `P255_CC_ZDG` の
ように既定 0 で残す価値が無い —— 既定形自体が遅くなるノブだからである）。

数値検証: 点変化係数（`SCALE_DG_VARYING_COEFF=1`）、`Ne=1`（16,777,216 点）、
参照は `CUDAFORTRAN_GEMM`。一般化 `TJ=4`・`TI=4/TJ=8`・本番のいずれも
max abs **3.553e-15**（§26.8 と同値）で、`TI=4/TJ=8` は本番と**全点ビット一致**。

### 27.4 天井を現行形で引き直す（job `78273`、c177、12 ラウンド交互）

§15.101 の残天井表は **1874 µs/stage 時点**の値である。§26.6 で −18.6% を
取ったあとの形で同じアブレーションを測り直した（`P255_CC_ABLATE` は
ツリーに元からあるノブ。1 = 内積 1 反復、3 = エピローグ剥がし、
4 = バリア全削除、5 = D≡1、6 = Q≡1。いずれも `dqdt` は壊れる天井専用）。

| アブレーション（不正） | 中央値 µs/stage | レンジ | 対本番 | §15.101 時点（1874 基準） |
|---|---:|---:|---:|---:|
| 本番（§26.6 の形） | **1532.8** | 1525.8–1535.3 | — | — |
| INNER=1（契約外） | 996.9 | 994.8–998.0 | **−34.96%** | −36.5% |
| Q≡1 | 1307.9 | 1306.2–1308.7 | **−14.67%** | −11.3% |
| エピローグ剥がし | 1358.4 | 1356.1–1359.3 | −11.38% | （§15.84 系列） |
| D≡1 | 1354.5 | 1353.1–1356.7 | **−11.63%** | −15.3% |
| バリア全削除 | 1475.8 | 1472.7–1481.2 | −3.72% | −2.7% |

全レンジ非重複。**天井の順位が入れ替わっている**: 改修前は
D（−15.3%）> Q（−11.3%）だったのが、いまは **Q（−14.7%）> D（−11.6%）** である。
機構は §26.7 の ncu と整合する —— 出力タイルは 1 スレッドが持つ `d[u]` の
再利用を 8 本に増やしたので **D 側の賞金が縮み**、一方 `f[a]` の側
（Q、すなわちフラックスの shared 平面と、それを埋める global ロード）は
4 本のままなので**相対的に大きくなった**。バリアの天井も 2.7% → 3.7% に
増えている（占有率が 48.7% → 18.1% になり、バリア 1 本の露出が上がった）。

### 27.5 判定

- **`p255_gap_study.md` §22（`FUSED_TC`）は維持する。** 出力タイル (A) の
  第 2 段（`TODO.md` §6 が挙げた新軸）をこの経路に当てて測り、
  iso 予算で **+0.33%**（返らない）であることを示した。機構は
  「(A) が割る相手が p=255 TC では屋根ではない」で、§22 の ncu が根拠。
- **`FUSED`（CC）には、まだ探索終了宣言を出さない。** §26.9 が残した 2 軸の
  うち **j タイル幅は測って閉じた**（§27.3、賞金無し）が、
  **ブロック形状 `(16,8)` は依然として未測定**である。これは具体的には
  「`tx` が 16 レーンで、x では D1D の連続 16 要素を読み、shared 平面の
  32 個の `l` を 2 回に分けて埋めている」形を `(32,4)` や `(8,16)` にする
  ことで、`(32,4)` にすると D1D のリクエストが 32 連続になる一方、
  1 ブロックが覆う j が半分になるので `TJ=8`（アキュムレータ 128 レジスタ）と
  セットでなければ同じブロック分割にならない。**残っているのはこの 1 軸だけ**で、
  §27.4 の天井（Q −14.7% / D −11.6% / バリア −3.7%）がその賞金の上限である。
- 現在地: `CUDAFORTRAN_FUSED`（CC）**1532.8 µs/stage**（job `78273` の本番行。
  §26.7 の 1525.0 とは 0.5% で一致し、別ノードの同一物差し）。
  **最速経路は `CUDAFORTRAN_FUSED_TC`（885.03、job `78266`）のまま。**

## 28. `FUSED`（CC）のブロック形状 —— 最後の 1 軸、**−5.60% で採用**と探索終了宣言（2026-09-03）

§27.5 は「残っているのはブロック形状 `(16,8)` の 1 軸だけ」と書いて終わって
いた。本節はその 1 軸を x / y / z の 3 本それぞれについて掃引し、
**y と z が `(8,16)` を取り（z は `minBlocks` 2 → 4 とセットで）、
x は `(16,8)` のまま**という結論を出す。あわせて **`FUSED`（CC）の探索終了
宣言**を出す。

### 28.1 測定条件と物差し

- 親 commit `3d4045c`（`feature/cuda`）＋本節の作業ツリー、GB200 1 GPU、
  `make CUDA=1 GPUFLAGS=-gpu=cc100`（`cuda_dg_kernels_fused_highp.cu` は
  `nvcc -O3 -std=c++17 -arch=sm_100`）
- 入力 `namelists/perf_p255_fused.conf`（`Ne=1`、`PolyOrder=255`、`nstep=20`、
  `UseCudaGraph=.false.` 固定）。検証は `namelists/val_p255_fused.conf` /
  `val_p255_gemm.conf` と、その `NeX/NeY/NeZ` だけを 2 にした作業コピー
  （コミットしない）
- 物差しは **`CUDA device fused tendency` ÷ 57**（§26.1 / §27.1 と同じ）
- 採否の A/B は **Slurm で占有した GPU 上で 12 ラウンド交互**、job **`78391`**
  （node **c186**）。天井の引き直しは job **`78396`**（node **c182**）。
  機構は ncu job **`78393`**（SOL / 占有率、c186）と job **`78394`**
  （明示メトリクス、c186）
- 掃引そのものは login node の 2-run（差が 1% 以上ある形の足切りにはこれで
  足りる。採否は必ず占有 GPU の A/B）
- HEAD の物差し合わせ: login 3-run で 1522.5 / 1526.6 / 1531.0 µs/stage、
  占有 GPU で **1525.50**（job `78391`）。§27.4 の 1532.8（c177）と 0.5% で
  一致する

> 実務上の注意（1 回失った）。最初の A/B（job `78390`）は全変種が空の結果で
> 返った。凍結実行ファイルをエージェントのスクラッチ（`/tmp` 配下）に置いて
> いたためで、**計算ノードからはそのパスが見えない**。凍結コピーは
> リポジトリ配下（`scale-dg_extraction.<tag>`、gitignore 対象外だが
> コミットしない）に置くこと。`AGENTS.md` が ncu の出力先について書いている
> のと同じ制約が、実行ファイル自身にも掛かる。

### 28.2 掃引のための一般化 —— **今回は代金が無い**

3 本のカーネルは `dim3(16, 8)` の 128 スレッドで、`threadIdx.x` が
「そのスレッドが持つ i（z では line）」、`threadIdx.y` が「4 本の j（k）」を
指していた。これを `P255_CC_{X,Y,Z}_BDX` でコンパイル時に振れるようにした
（`BDY = 128 / BDX`）。共有パネルの l 方向の深さ `P255_CC_{Y,Z}_LT` も
同時にノブにした（y は `BDX=32` で静的 shared 48 KB を超えるため、`LT` を
半分にしないと**そもそも組めない**）。y / z の shared D パネルの
「2*(tx&7) + (tx>>3)」ペアスウィズル（§15.12）は
`P255_JPOS(m, BDY)` に一般化した —— 4 本の j が `2*ty` と `2*BDY + 2*ty` の
2 本の `double2` で返るように m を並べ替える写像である。

**§27.3 と違って、この一般化には代金が無い。**

| 変種 | 中央値 µs/stage | レンジ | 対 HEAD |
|---|---:|---:|---:|
| HEAD（手書き `dim3(16,8)`） | 1525.50 | 1522.76–1529.11 | — |
| 一般化コードの `BDX=16`（同じ形） | **1519.80** | 1517.25–1523.85 | **−0.37%** |

レンジは重ならない。§27.3 の `TJ` 一般化は本番形を +0.79% 落としたが、
`BDX` の一般化は逆に 0.37% **速い**（SASS は別物になるので符号はどちらにも
振れる、というのが §27.3 と本節を合わせた読み方である）。したがって
「一般化を残すか手書きに畳むか」は本節では争点にならない。**採用形は
`#define` の既定値としてコンパイル時に固定した**（`constexpr` に畳まれるので
実行時の分岐は無い）。

なお掃引の途中で `LT % BDY == 0` を満たさない形（z の `BDX=8` × `LT=8`）を
組んでしまい、`NROW = LT/BDY = 0` で **sQ 充填ループが 1 回も回らない**まま
1008 µs という「速い」数字が出た。数値は当然壊れている。同じ事故を防ぐため
3 本に `static_assert` を入れた（`LT % BDY == 0`、`32 % BDX == 0`、
D パネル充填のスレッド整合）。**測定値が天井（INNER=1 の 820 µs）に近づいたら
まず形の妥当性を疑うこと。**

### 28.3 掃引（login 2-run、他の 2 本は本番形に固定）

分母は一般化コードの `BDX=16`（login 1522.2 / 1523.1）。

**x**（`TI=8`、`TJ=4`、`MINB=3`）:

| x の形 | shared | レジスタ | login µs/stage | |
|---|---:|---:|---:|---|
| `(16,8)`（本番） | 8448 B | 167 | 1522.2 / 1523.1 | — |
| `(32,4)` | 4224 B | 168 | 1526.1 / 1533.1 | 差なし〜微減速 |
| `(8,16)` | 16896 B | 167 | 1568.5 / 1566.8 | **+3.0%** |
| `(4,32)` | 33792 B | 168 | 1810.2 / 1811.5 | **+19%** |

`(32,4)` は `MINB` も振り直した（本節の採用形 y/z の上で、login 2-run）:
`MINB=3` 1459.2 / 1447.7、`MINB=2` 1492.2 / 1493.7、`MINB=4` 1523.0 / 1477.5。
どれも本番（1440.6 / 1441.0）に届かない。**x は `(16,8)` のまま。**

**y**（`TI=8`、`MINB=2`）:

| y の形 | `LT` | shared | レジスタ | login µs/stage | |
|---|---:|---:|---:|---:|---|
| `(16,8)`（本番） | 16 | 40960 B | 168 | 1522.2 / 1523.1 | — |
| **`(8,16)`** | 16 | **32768 B** | **156** | **1512.3 / 1512.8** | **−0.7%** |
| `(16,8)`、`LT=8`（`LT` 単独の対照） | 8 | 20480 B | — | 1601.8 / 1604.2 | **+5.3%** |
| `(32,4)`（`LT=8` 必須） | 8 | 34816 B | — | 1643.0 / 1647.2 | +8.2%（うち `LT` が +5.3%） |
| `(8,16)`、`TI=4` | 16 | 24576 B | — | 1576.7 / 1576.9 | +3.6% |
| `(8,16)`、`MINB=1` / `3` | 16 | 32768 B | 156 | 1511.1 / 1511.8 | 差なし |

**y の `(32,4)` は 1 軸の変更にならない。** sQ は `TI*2*LT*BDX` doubles なので
`BDX=32` は 64 KB を要求し、静的 shared 48 KB の壁を超える。`LT` を 8 に
落とせば組めるが、その `LT=8` 自体が単独で +5.3% であり、`(32,4)` の 8.2% の
うち 5.3% は `LT` の代金である。**残る 2.9% が形の代金**で、いずれにせよ負け。
`(4,32)` は `LT % BDY` を満たせず（`LT=32` にすると sD が 64 KB）組めない。
**これは「測っていない」ではなく「静的 shared の壁で存在しない」形である。**

**z**（`TL=8`）:

| z の形 | `LT` | `MINB` | shared | レジスタ | login µs/stage | |
|---|---:|---:|---:|---:|---:|---|
| `(16,8)`（本番） | 16 | 2 | 20480 B | 166 | 1522.2 / 1523.1 | — |
| **`(8,16)`** | 16 | 2 | **16384 B** | **146** | **1464.4 / 1462.4** | **−3.9%** |
| **`(8,16)`、`MINB=4`** | 16 | 4 | 16384 B | **128** | **1452.4 / 1448.5** | **−4.8%** |
| `(8,16)`、`MINB=1` / `3` / `5` | 16 | — | — | — | 1457.6 / 1456.5 / 1461.6 | `MINB=4` に劣る |
| `(8,16)`、`MINB=6` | 16 | 6 | — | — | 1899.6 / 1908.6 | **+25%**（予算が足りない） |
| `(32,4)` | 16 | 2 | 34816 B | — | 1895.1 / 1893.0 | **+24%** |
| `(8,16)`、`LT=32` | 32 | 2 | 32768 B | — | 1542.9 / 1537.1 | +1.0% |
| `(4,32)`、`LT=32` | 32 | 2 | 40960 B | — | 1550.1 / 1552.4 | +1.9% |
| `(16,8)`、`LT=32`（`LT` 単独の対照） | 32 | 2 | 40960 B | — | 1668.6 / 1667.1 | **+9.6%** |
| `(8,16)`、`TL=16` | 16 | 2 | — | — | 1543.3 / 1540.2 | +1.3% |
| `(8,16)`、`TL=4` | 16 | 4 | — | — | 1518.9 / 1517.9 | +0.3% |

**z は `(8,16)` ＋ `MINB=4`。** 出力数（`TL=8` × 4 k ＝ 32）は動かしていない
—— `TL=16`（64 出力）も `TL=4`（16 出力）も新しい形の上で負ける。
§26.6 / §27.3 の「梯子は 32 出力で頭打ち」は新しい形でも成り立つ。

### 28.4 占有 GPU A/B（job `78391`、node c186、12 ラウンド交互）

| 変種 | 中央値 µs/stage | レンジ | 対 HEAD | 対一般化 `BDX=16` |
|---|---:|---:|---:|---:|
| HEAD | 1525.50 | 1522.76–1529.11 | — | +0.38% |
| 一般化 `BDX=16`（同形） | 1519.80 | 1517.25–1523.85 | −0.37% | — |
| x `(32,4)` | 1525.11 | 1523.97–1534.58 | −0.03%（**レンジ重複＝差なし**） | +0.35% |
| x `(8,16)` | 1559.61 | 1557.42–1563.76 | **+2.24%** | +2.62% |
| y `(8,16)` | 1511.16 | 1510.08–1516.21 | **−0.94%** | −0.57% |
| z `(8,16)`、`MINB=2` | 1457.25 | 1454.14–1461.36 | **−4.47%** | −4.12% |
| z `(8,16)`、`MINB=4` | 1448.89 | 1443.51–1457.00 | **−5.02%** | −4.67% |
| **y `(8,16)` ＋ z `(8,16)`/`MINB=4`（採用）** | **1440.04** | 1439.36–1445.81 | **−5.60%** | −5.25% |

x `(32,4)` を除いて全レンジ非重複。**y と z の効果は加法的**
（−0.94 −5.02 = −5.96% に対し実測 −5.60%）で、3 本が直列に走る以上これは
期待どおりである。

### 28.5 機構 —— **占有率ではなく DRAM バイトが半分になっている**

作業仮説は「z は DRAM レイテンシ律速なので占有率を買うのが効き、x / y は
L1/TEX 律速なので効かない」だった。ncu（job `78393` の SOL / 占有率、
job `78394` の明示メトリクス。どちらも c186、`--kernel-name regex:` で
3 本を同一ジョブに）は、**この仮説を半分だけ支持し、半分は否定する。**

ncu job `78394`（`-s 3 -c 6`、3 本 × 2 launch の中央値。x は採用形でも
`(16,8)` のままなので、この列は「変えていない側の対照」である）:

| メトリクス | x.16,8 | x.採用 | y.16,8 | y.採用 | z.16,8 | z.採用 |
|---|---:|---:|---:|---:|---:|---:|
| `dram__bytes_read.sum` | 4.045e8 | 4.044e8 | 5.387e8 | 5.390e8 | **2.425e9** | **1.347e9 (−44.5%)** |
| DRAM 対ピーク % | 9.03 | 8.99 | 9.71 | 9.50 | **37.75** | **21.66** |
| L2 sector hit % | 55.78 | 55.91 | 67.86 | 66.38 | **10.50** | **33.27** |
| global ld sector | 1.564e8 | 1.564e8 | 8.507e7 | **5.938e7 (−30%)** | 8.598e7 | **6.029e7 (−30%)** |
| global ld request | 3.657e7 | 3.657e7 | 1.088e7 | **7.733e6 (−29%)** | 1.180e7 | **8.651e6 (−27%)** |
| L1/TEX 対ピーク % | 79.23 | 79.34 | 86.28 | 87.37 | 85.71 | **91.29** |
| shared wavefront | 1.930e7 | 1.930e7 | 9.499e7 | 9.675e7 (+1.9%) | 9.475e7 | 9.725e7 (+2.6%) |
| shared st バンクコンフリクト | 2.784e5 | 2.815e5 | 4.471e5 | **4.265e6 (×9.5)** | 1.586e5 | **4.382e6 (×27.6)** |
| `mio_throttle` | 0.04 | 0.04 | 0.35 | **0.63** | 0.43 | **1.26** |
| `long_scoreboard` | 1.655 | 1.650 | 2.540 | 2.465 | 3.290 | 3.580 |
| `barrier` | 0.05 | 0.05 | 0.18 | 0.16 | 0.16 | **0.32** |
| 命令数 | 1.966e8 | 1.966e8 | 2.145e8 | 2.089e8 (−2.6%) | 2.085e8 | 2.048e8 (−1.8%) |
| 占有率（達成） | 18.09 | 18.09 | 17.96 | 18.09 | **18.08** | **23.89** |
| ncu Duration (ns) | 7.232e5 | 7.252e5 | 8.472e5 | 8.718e5 | 8.522e5 | 8.556e5 |

レジスタ / 静的 shared（ptxas）: x 167 / 8448 B（不変）、
y 168 → **156** / 40960 → **32768 B**、z 166 → **128** / 20480 → **16384 B**。
spill は全形 0。z の Block Limit Registers は 3 → 4 に増える（job `78393`）。

**支配的なのは占有率ではなく、DRAM が動かすバイトである。**
z の DRAM read が **2.425e9 → 1.347e9（−44.5%）**、L2 hit が 10.5% → 33.3% と
いうのが本節で一番大きい数字で、占有率の 18.1% → 23.9% はそれに付随する。
機構は数え上げで閉じる:

- 3 本とも、共有パネルに載せるフラックス `q*velocity` の添字は**そのカーネルが
  縮約する方向に依存しない**。x は `(l, j, k)` で i に依存せず、
  y は `(i, l, k)` で j に依存せず、z は `(line, l)` で k に依存しない。
- したがって **`q` / `velocity` を DRAM から読み直す回数は、
  「フラックスが依存しない方向」に沿ったブロック数に等しい**。
  x ではそれが i 方向のブロック数 `NPAIR = 256/(BDX*TI)`、
  y / z では j / k 方向のブロック数 `256/(4*BDY)` である。
- `BDX` を 16 → 8 にすると、**y / z では読み直し回数が 8 → 4 に半減し、
  x では 2 → 4 に倍増する。** 符号が逆なのは、x だけ「フラックスが依存しない
  方向」が `threadIdx.x` に乗っているからである。
- 実測はこの数え上げどおり: z の DRAM read はほぼ半分になり
  （`q` と `velocity` の 2 本が支配的で、`dqdt` / `Escale` / `flux_bnd` は
  読み直されないので厳密な 1/2 にはならない。予想 0.56 倍、実測 0.555 倍）、
  x の `(8,16)` は占有 GPU で **+2.24%** 負ける。

**y で DRAM バイトが動かないのは、y の読み直しがもともと L2 に収まって
いるからである。** y の global ld sector は z と同じく −30% 減っているのに
DRAM read は 5.387e8 → 5.390e8 で**動かない**（L2 hit 67.9% → 66.4%、
これも動かない）。y が触る面（`i × l`、固定 k）は 512 KB で L2 に載るが、
z が触る面（`line × l`、l が 65536 doubles ストライド）は要素全体
128 MB に散るので L2 hit が 10.5% しかない。**同じ −30% の L1 リクエスト削減が、
z では DRAM まで届き、y では L2 で止まる。** これが z −5.02% 対 y −0.94% の
比の説明であり、「z が DRAM 側で詰まっている」という仮説の当たっていた部分
である。

**一方、仮説の「占有率を買うのが効いた」という部分は否定される。**
`MINB` 2 → 4 の寄与は −5.02 − (−4.47) = **−0.55%** しかなく、
本体の −4.47% は `MINB=2` のまま、すなわち**占有率 18% のまま**得られている。
レジスタが 166 → 146 → 128 と落ちるのは形の副産物であって原因ではない。
`p127_gap_study.md` §23.9 の「塞がっていたのは占有率であって性能では
なかった」は、**p=255 CC のブロック形状では成り立たない**（§26.3 / §26.4 /
§27.2 に続く 4 例目の反例。ただし今回は「占有率を下げて予算を作る」ではなく
「占有率を上げる」向きの反例である）。

**採用形は shared 側では明確に悪くなっている。** shared の store バンク
コンフリクトが y ×9.5・z ×27.6 に増え（sD 充填が `P255_JPOS` の一般形に
なったこと、および sQ の行幅が 16 → 8 doubles に狭まったことによる）、
`mio_throttle` は z で 0.43 → 1.26 に、`barrier` も 0.16 → 0.32 に増える。
L1/TEX は 85.7% → 91.3% と**屋根に近づく**。それでも 5.60% 速い。
**DRAM から 1.08 GB/stage 消したことが、shared パイプに移した代金を上回る**
という取引である。ncu の Duration が z で 8.522e5 → 8.556e5 ns と**逆符号**に
出ているのは `AGENTS.md` が書いているとおりで（クロック固定で global の待ちが
サイクル数として安く見える）、**ncu の時間で採否を決めていたらこの 5.60% は
落としていた**。`p63_gap_study.md` §19.10 / §20.3 / §20.6 の 6 例に続く 7 例目
であり、**DRAM バイトを実際に減らした変更で ncu が逆符号を出した例**という
点で新しい。

測っていないもの: x の `(8,16)` / `(32,4)` には ncu を当てていない
（採用形では x は `(16,8)` のままなので、上表の x 列は対照である）。
x が負ける理由は上の数え上げと占有 GPU の実時間から述べたものであって、
**x の DRAM バイトが実際に倍増したことを測ってはいない。**

### 28.6 §27.3 の規則はブロック形状には及ばない —— **判定**

§27.3 は「(A) の梯子は**出力数**で決まり、その形（i と j の配分）では
決まらない」と結論した。本節の眼目は、この規則がブロック形状にも及ぶか
どうかだった。**及ばない。**

- §27.3 が振った `TI × TJ`（1 スレッドが持つ出力の配分）は、
  **ブロックが覆う範囲を変えない**。8×4 も 4×8 も 1 ブロックが覆う
  (i, j) は同じで、global の読み直し回数も同じである。だから出力数だけが効いた。
- 本節が振った `BDX × BDY`（ブロックの形）は、**同じ出力数のまま
  ブロックが覆う範囲を変える**。`BDX` を半分にすると i 方向のブロックは
  倍に、j / k 方向のブロックは半分になり、**フラックスの読み直し回数
  そのものが動く**。動くのは L1 の下、DRAM である。
- したがって規則はこう書き直される: **1 スレッドの中の配分は出力数だけで
  効き、ブロックの中の配分は「フラックスが依存しない方向のブロック数」で
  効く。** 前者は L1/TEX の上の話、後者は DRAM の話であって、
  同じ「i と j の配分」でも層が違う。

これは §27.3 の結論を覆すものではない（§27.3 が振った軸については
§27.3 の結論のままである）。**「i / j の配分は効かない」を
ブロック形状まで外挿していたら 5.60% を落としていた**、という意味で、
本節は §27.5 が「残っている」と書いた判断が正しかったことの実例でもある。

### 28.7 採用形と数値検証

採用は **x: `dim3(16,8)`（据え置き、`TI=8` / `TJ=4` / `MINB=3` / 167 reg）、
y: `dim3(8,16)`（`TI=8` / `MINB=2` / 156 reg / shared 32 KB）、
z: `dim3(8,16)`（`TL=8` / `MINB=4` / 128 reg / shared 16 KB）**。
`cuda_dg_kernels_fused_highp.cu` の `P255_CC_{X,Y,Z}_BDX` の既定値を
16 / 8 / 8 に、`P255_CC_Z_MINB` を 2 → 4 に変更した。すべて `constexpr` に
畳まれるので実行時の分岐は無い。`P255_CC_{Y,Z}_LT` は 16 のまま残した
（`BDX=32` を測るのに必要で、既定形自体は遅くならないため。§27.3 の `TJ`
ノブを捨てた理由—既定形が遅くなる—は本節には当てはまらない）。

`SCALE_DG_VARYING_COEFF=1`、`SCALE_DG_DUMP_DQDT` で owned `dqdt(:,1:Ne)` 全点:

| 比較 | 点数 | 結果 |
|---|---:|---|
| 採用形 対 HEAD（`Ne=1`） | 16,777,216 | **全点ビット一致** |
| 採用形 対 HEAD（`Ne=2³`） | 134,217,728 | **全点ビット一致** |
| 採用形 対 `CUDAFORTRAN_GEMM`（`Ne=1`） | 16,777,216 | max abs **3.553e-15**、>1e-14 が 0 件 |
| 採用形 対 `CUDAFORTRAN_GEMM`（`Ne=2³`） | 134,217,728 | max abs **3.553e-15**、>1e-14 が 0 件 |
| 一般化 `BDX=16` 対 HEAD（`Ne=1`） | 16,777,216 | **全点ビット一致** |
| 掃引した各形（x32 / x8 / y8 / z8）対 HEAD（`Ne=1`） | 16,777,216 | **いずれも全点ビット一致** |

ブロック形状はスレッドと出力の対応を変えるだけで、**内積の加算順も
FMA の縮約も変えない**ので、掃引した全形が HEAD とビット一致である。
p=255 に `CUDAFORTRAN_SPLIT` は無いので参照は `GEMM`（§25 / §26.8 と同じ）。

### 28.8 天井を採用形で引き直す（job `78396`、node c182、12 ラウンド交互）

`P255_CC_ABLATE`（1 = 内積 1 反復、3 = エピローグ剥がし、4 = バリア全削除、
5 = D≡1、6 = Q≡1。いずれも `dqdt` は壊れる天井専用）:

| アブレーション（不正） | 中央値 µs/stage | レンジ | 対本番 | 絶対 µs | §27.4（1532.8 基準） |
|---|---:|---:|---:|---:|---:|
| 本番（§28.7 の採用形） | **1441.25** | 1439.33–1443.23 | — | — | 1532.8 |
| INNER=1（契約外） | 820.70 | 818.64–821.15 | **−43.06%** | 620.6 | −34.96%（535.9 µs） |
| D≡1 | 1228.84 | 1225.96–1230.28 | **−14.74%** | 212.4 | −11.63%（178.3 µs） |
| Q≡1 | 1256.36 | 1252.44–1258.30 | **−12.83%** | 184.9 | −14.67%（224.9 µs） |
| エピローグ剥がし | 1348.81 | 1347.90–1351.71 | −6.41% | 92.4 | −11.38%（174.4 µs） |
| バリア全削除 | 1386.72 | 1385.61–1389.24 | −3.78% | 54.5 | −3.72%（57.0 µs） |

全レンジ非重複。**§27.4 で入れ替わっていた D と Q の順位が、また入れ替わって
元に戻っている**（`D −14.74% > Q −12.83%`）。機構は §28.5 と整合する:

- **Q（フラックス）の天井は 224.9 → 184.9 µs に縮んだ。** これは
  読み直し回数が半減して DRAM から消えた分を、本節がすでに現金化したから
  である。**残っている Q の賞金は L1/shared 側の分**である。
- **エピローグの天井は 174.4 → 92.4 µs に縮んだ。** `Escale` / `dqdt` /
  `flux_bnd` のアクセスも j / k 方向のブロック数に沿って読み直されるので、
  ここも同じ機構で半分近く消えた（L2 hit 10.5% → 33.3% の一部）。
- **D の天井は 178.3 → 212.4 µs に増えた。** D 側は形の影響を受けないので
  絶対値が増えたのは shared 側の代金（バンクコンフリクト ×27.6、
  `mio_throttle` ×2.9）で、これは §28.5 が「shared パイプに移した代金」と
  書いたものと同じ物である。同じ理由で INNER=1 の賞金も 535.9 → 620.6 µs に
  増えている。
- バリアの天井は 57.0 → 54.5 µs でほぼ不変。

### 28.9 `FUSED`（CC）の探索終了宣言

**`p255_gap_study.md` は、ここで `CUDAFORTRAN_FUSED`（CC、p=255）の
探索終了を宣言する。1441.25 µs/stage（job `78396`）/ 1440.04 µs/stage
（job `78391`）が到達点である。**

`TODO.md` §6 の 5 例が示したとおり、覆った宣言はいずれも
「**当時の候補集合に、後で他次数から来る軸が入っていなかった**」ために
覆った。したがってこの宣言も、**将来覆る可能性を否定しない**。
これは「2026-09-03 の時点で列挙できた候補が尽きた」という意味の宣言であり、
**何を測ったうえでの宣言かを以下に明示する**（覆すときは、この一覧に
入っていない軸を持ってくることになる）。
...
| 軸 | どこで | 結果 |
|---|---|---|
| 1 点/スレッド → 2 点、フラックス共有、`__ldg` / `__ldcs` / `__ldca` / `__ldcg` / `__ldlu`、`__stwb` / `__stcs` / `__stcg`、`cp.async` 先読み、`double2` 化、行ストライド、列 XOR スウィズル、二重バッファ、末尾バリア、`cudaSharedmemCarveout`、ソフトウェアパイプライン、`unroll` 各種、x+y 融合、l タイル幅 16/32/64 | §15.1–§15.101（採用 14 / 不採用 30 超） | 1874.2 µs まで。**§15.101 が「契約内の残りは測り尽くした」と書いたが、宣言は出していなかった** |
| 出力タイル (A) × レジスタ予算 (B) の第 2 段 | §26.6–§26.7 | **採用 −18.63%**（1874.3 → 1525.0）。梯子は `TI=16` で終わる |
| z の D の global 化（`__ldg`） | §26.5 | 不採用 +11.26% |
| 1 スレッド内の j タイル幅 `TJ`（`TI×TJ` = 32 / 64 の全配分） | §27.3 | 採用ゼロ。梯子は**出力数**で決まる |
| 天井の引き直し（1532.8 基準） | §27.4 | INNER=1 −34.96% / Q −14.67% / D −11.63% / エピローグ −11.38% / バリア −3.72% |
| **ブロック形状 `(BDX, 128/BDX)`、3 本それぞれ、`BDX` = 4 / 8 / 16 / 32** | **§28.3–§28.4** | **採用 −5.60%**（y / z が `(8,16)`、z は `MINB=4` とセット。x は据え置き） |
| ブロック形状と交差する `MINB` 再調整（x `(32,4)` で 2/3/4、y `(8,16)` で 1/2/3、z `(8,16)` で 1〜6） | §28.3 | z の `MINB=4` だけ採用（−0.55%）。他は差なしか負け |
| ブロック形状と交差する `LT`（共有パネルの l 深さ、y 8/16、z 16/32） | §28.3 | 採用ゼロ（y `LT=8` +5.3%、z `LT=32` +9.6%） |
| ブロック形状と交差する出力数（z `TL` = 4 / 8 / 16、y `TI` = 4 / 8） | §28.3 | 採用ゼロ。**32 出力の頭打ちは新しい形でも動かない** |
| 天井の引き直し（1441.25 基準） | §28.8 | INNER=1 −43.06% / D −14.74% / Q −12.83% / エピローグ −6.41% / バリア −3.78% |

**残る天井と、それが契約内で取れない理由:**

- **INNER=1 の −43.06%（620.6 µs）は契約外**である。長さ 256 の内積そのもの
  で、`AGENTS.md` の数値コントラクトが要求する `D(q*u)` の定義そのものを
  減らすことになる。
- **D≡1 の −14.74%（212.4 µs）と Q≡1 の −12.83%（184.9 µs）は、
  いずれも「オペランドを 1 つ消す」天井であって、実装で取れる賞金では
  ない。** この 2 つに対して §15 と §26–§28 が当てた実装候補
  （キャッシュ修飾子 7 種、shared 化 / global 化、`double2`、
  スウィズル、レジスタ常駐、ソフトウェアパイプライン、二重バッファ、
  出力タイル、ブロック形状）はすべて測ってあり、**採用されたのは
  出力タイル（−18.63%）とブロック形状（−5.60%）の 2 つだけ**である。
  残りの 212.4 + 184.9 µs は「読み直しを減らす」のではなく
  「読むこと自体をやめる」分であり、契約内では届かない。
- **エピローグの −6.41%（92.4 µs）とバリアの −3.78%（54.5 µs）は、
  §15.89（末尾バリア省略）・§15.92（y 二重バッファ）ですでに一部を現金化
  してあり、残りは x / z の二重バッファ（§15.93 / §15.94、いずれも負け）
  と `Escale` / `flux_bnd` のキャッシュ修飾子（§15.39–§15.41、§15.53、
  §15.97–§15.98、採用 1 / 不採用 5）で叩いてある。**
- したがって、**§28.8 の天井のうち契約内で残っている賞金は、
  測定誤差（レンジ幅で ±0.15%）を超えて取れる形が現時点で 1 つも
  列挙できない。** これが宣言の根拠である。

**この宣言が意味しないこと:**

- 「CC が最速だ」とは言っていない。**最速経路は `CUDAFORTRAN_FUSED_TC`
  （883.55 / 885.03 µs/stage）のまま**であり、CC は `GEMM_FUSED`
  （§10 系列の 943–963 µs/stage）にも届かない。主比 B は
  1440.04 / 883.55 = **1.630×**（§26.9 の 1.726× から縮んだ）。
- 「CC を TC の実装に寄せない」という `AGENTS.md` の役割規定は変わらない。
  `FUSED_DFMA` が iso-schedule のアブレーションとして別に存在する以上、
  TC のフラグメントレイアウトを `FUSED` に持ち込むことは**この宣言の
  射程の外**であり、候補集合にも入れていない。
- 将来この宣言が覆るとすれば、それは他次数から来る**まだ列挙していない軸**
  によってである。`TODO.md` §6 の 5 例と同じ形で、そのときは
  「§28.9 の候補集合に入っていなかった」と書けばよい。

### 28.10 到達点

- `CUDAFORTRAN_FUSED`（CC、p=255）: **1525.0 → 1440.04 µs/stage（−5.60%）**。
  §26.1 の物差しで 26.11 GFLOP/stage なら **18.13 TFLOP/s = 40.1 の 45.2%**
  （§26.9 時点は 17.12 TFLOP/s、42.7%）。1874.3 からは通算 **−23.2%**。
- **主比 B（`FUSED` / `FUSED_TC`）は 1.726× → 1440.04 / 883.55 = 1.630×。**
- **最速経路は `CUDAFORTRAN_FUSED_TC` のまま。** `FUSED_TC` は §27.2 で
  探索終了宣言を維持しており、本節では触っていない。
- `TODO.md` §6 の「`p255_gap_study.md` に `FUSED` の探索終了宣言が無い」は
  **§28.9 で閉じた**。

## 29. `Nq=256` の nsys 分解 —— **「手書き mainloop の屋根 80%」は次数によらない定数ではない**（2026-09-03）

`p511_gap_study.md` §15.6 が「残っている未説明」として明記した 1 件
（*`Nq=256` で同じ 3 本 + `volume_flux` の nsys 分解を取っていないので、
手書き mainloop の屋根が p=255 でも 80% だったのかは測っていない*）を、
実際に分解して閉じる。**逆算ではなく分解である。**

### 29.1 測定条件

- ツリー: **`44b02a6`**（`feature/cuda`）。凍結実行ファイル
  `scale-dg_extraction.unexp3`（リポジトリ配下）。
  `make CUDA=1 GPUFLAGS=-gpu=cc100`、NVIDIA HPC SDK 26.3。
- Slurm job **`78530`**、node **`c187`**、GB200 1 GPU（`clocks.max.sm` 2062 MHz、
  persistence on、compute mode Default）。
- 入力: `namelists/perf_p255_fused_tc.conf` と、その `DqdtKernel_Type` だけを
  `CUDAFORTRAN_GEMM_FUSED` に替えたスクラッチ複製 `jobs/perf_p255_gemm_fused.conf`。
  **`NeX/NeY/NeZ = 1`・`PolyOrder = 255`・`dt = 1.0D-7`・`nstep = 20`・
  `UseCudaGraph = .false.` は 2 本で同一**、変えていない。
- 物差し: 実行ファイルの `CUDA device *` 行 ÷ (`Measured steps` 19 × 3) ＝ µs/stage。
  採否 A/B は**同一ジョブ内 12 ラウンド交互**（PART A）。
- nsys は同 job の PART B、`nstep=4` に落とした複製に対して
  `nsys profile --resolve-symbols=false -t cuda -f true`、`DEBUGINFOD_URLS=` 空、
  `timeout 600`。集計は `nsys stats --report cuda_gpu_kern_sum --format csv` の
  **Med (ns)**（12 インスタンス = 4 step × 3 stage）。

### 29.2 実時間（PART A、12 ラウンド交互、µs/stage）

| 経路 | 中央値 | レンジ | 対 `GEMM_FUSED` | 対ピーク（2.611e10 FLOP/stage ÷ 40.1 TFLOP/s）|
|---|---:|---:|---:|---:|
| **`CUDAFORTRAN_FUSED_TC`** | **880.5** | 879.2–881.2 | **−6.36%** | **73.9%** |
| `CUDAFORTRAN_GEMM_FUSED` | 940.3 | 939.5–942.3 | — | 69.3% |

**レンジ非重複。** `FUSED_TC` の 880.5 は §28.10 が引いた 883.55、
§22 の 883.781 と 0.4% 以内で一致する。`GEMM_FUSED` の 940.3 は §10 系列の
943–963 µs/stage の下端で、§29.4 で使うのはこの現行値である。

### 29.3 nsys 分解（PART B、Med、µs/stage）

**`CUDAFORTRAN_FUSED_TC`:**

| カーネル | 時間 [µs] |
|---|---:|
| `tendency_p255_kernel<0(x), 256, TC>` | 289.008 |
| `tendency_p255_kernel<2(z), 256, TC>` | 283.536 |
| `tendency_p255_kernel<1(y), 256, TC>` | 282.064 |
| **3 本合計** | **854.608** |
| `elembnd_flux_group_kernel`（side stream） | 19.680 |

**`CUDAFORTRAN_GEMM_FUSED`:**

| カーネル | 時間 [µs] |
|---|---:|
| z `GemmBatchedDqdtAssembly` 64×32（融合エピローグ） | 279.728 |
| x CUTLASS `Gemm` 64×128 | 254.384 |
| y `GemmBatchedScaleAdd` 64×64 | 241.872 |
| **volume GEMM 3 本合計** | **775.984** |
| `volume_flux_kernel` | **128.016** |
| `elembnd_flux_kernel`（side stream） | 27.680 |
| **3 GEMM + flux** | **904.000** |

帰属（§15.6 と同じ形で）:

| 項目 | µs/stage | `GEMM_FUSED` の 904.000 に対して |
|---|---:|---:|
| 融合が**吸収した** `volume_flux` | **−128.016** | **−14.16%** |
| 手書き mainloop が**払った超過**（854.608 − 775.984） | **+78.624** | **+8.70%** |
| 差引 | **−49.392** | **−5.46%** |

実測の wall 差は **−6.36%**（§29.2）。**符号は一致し、大きさは 0.9 ポイント
食い違う。** 食い違いの中身は分解の外側にある残差で、
`GEMM_FUSED` は 940.3 − 904.000 = **36.3 µs**、`FUSED_TC` は
880.5 − 854.608 = **25.9 µs** を 3 本の外に持っている（launch 間隙、halo、
`rk_update` の前後）。その差 10.4 µs が 940.3 の 1.1% で、
0.9 ポイントの食い違いはここで説明がつく。**`elembnd_flux` は 2 経路とも
side stream にあり、どちらの critical path にも入っていない**ので帰属に
使っていない（TC 19.680 / GEMM 27.680）。

### 29.4 屋根 —— **80% は Nq=512 での値であって、次数によらない定数ではない**

1 本の volume GEMM の演算下限は `2·Nq⁴ / 40.1e12`。
`Nq=256` では **214.21 µs/本**（`Nq=512` は 3.427 ms/本）。

| | 演算下限 | 実測 | 屋根に対して |
|---|---:|---:|---:|
| **`FUSED_TC` x / z / y（Nq=256）** | 214.21 µs | 289.008 / 283.536 / 282.064 | **74.1 / 75.5 / 75.9%** |
| **`GEMM_FUSED` z / x / y（Nq=256）** | 214.21 µs | 279.728 / 254.384 / 241.872 | **76.6 / 84.2 / 88.6%** |
| `FUSED_TC` z / x / y（Nq=512、§15.6） | 3.427 ms | 4.2893 / 4.2755 / 4.2709 | 79.9 / 80.2 / 80.2% |
| `GEMM_FUSED` z / x / y（Nq=512、§15.6） | 3.427 ms | 3.8658 / 3.6711 / 3.5877 | 88.6 / 93.3 / 95.5% |

（`FUSED_TC` の値は §15.6 と同じく **GEMM 分の FLOP だけを分子にした下限側**
である。3 本の中では flux とエピローグもやっている。）

**答え: 屋根は次数によらない定数ではない。**

| | 3 本平均（Nq=256） | 3 本平均（Nq=512） | 増分 |
|---|---:|---:|---:|
| 手書き mainloop（`FUSED_TC`） | **75.2%** | **80.1%** | **+4.9 pt** |
| CUTLASS（`GEMM_FUSED`） | **83.1%** | **92.5%** | **+9.4 pt** |
| **CUTLASS − 手書き** | **+7.9 pt** | **+12.4 pt** | **+4.5 pt** |

したがって §15.6 の「手書き mainloop は mma 屋根の 80% で頭打ち」という
1 行は、**Nq=512 で観測した 1 点を屋根と呼んでしまっている**。実際には
両方の曲線が Nq とともに上がっており、**手書きの方が上がり方が遅い**。
機構としてより正確な言い方は次のとおりで、これは §15.6 の結論
（リードが Nq=256 と 512 の間でゼロを横切る）を弱めるのではなく、
**2 本の曲線の話に置き換えて強くする**:

- **賞金は `1/Nq` で縮む。** 融合が吸収する `volume_flux` の stage 比は
  Nq=256 で **14.16%**、Nq=512 で **8.36%**。比は 0.59 で、`1/Nq`（0.5）に
  近い（`volume_flux` は Np に比例し、GEMM は Nq·Np に比例するため）。
- **超過は広がる。** 手書き mainloop の CUTLASS に対する効率差は
  **7.9 → 12.4 ポイント**。
- **交差はこの 2 本が決める。** Nq=256 では賞金 14.16% が超過 8.70% を
  上回って −5.46%、Nq=512 では賞金 8.36% が超過 14.07% に負けて +5.54%。
  **どちらの次数でも、帰属は実測 wall 差と 1 ポイント以内で閉じている。**

### 29.5 `FUSED_TC` が p=255 で勝っている内訳（依頼の 3 番目）

§29.3 の表がそのまま内訳である。`FUSED_TC` は p=255 で
`GEMM_FUSED` に **−6.36%**（−59.8 µs/stage）勝つが、その内訳は

- **`volume_flux` を丸ごと消した −128.0 µs（−14.16%）** が唯一の得で、
- **mainloop で +78.6 µs（+8.70%）払っている**、

という差引である。**勝ちは mainloop が速いからではない。**
3 本の GEMM 部分だけを取れば `FUSED_TC` は CUTLASS に 10.1% 負けており
（854.608 対 775.984）、p=255 の勝敗を決めているのは
**エピローグ融合が消す 1 本のカーネル**である。同じ構造が p=511 では
賞金が半分になって符号が反転する（§15.6）。

### 29.6 §15.6 の訂正と、閉じた項目

- **§15.6 の測定表は書き換えない。** Nq=512 の 79.9/80.2/80.2% と
  88.6/93.3/95.5% はそのまま正しい。
- **訂正するのは「屋根 80%」という読み方だけ。** 正しくは
  「Nq=512 で 80.1%、Nq=256 で 75.2% の、Nq とともに上がる曲線」である。
- **§15.6 の「残っている未説明」は本節で閉じた。** `Nq=256` の分解を取り、
  屋根が次数に依存することを測定で示した。
- **性能上の変更は本節には無い。** 追加したのは測定だけである。
- 付随して、§15.6 の 6 次数リード表の p=255 行は現行ツリーで
  **`GEMM_FUSED` 69.3% / `FUSED_TC` 73.9% ＝ リード +4.7 pt** になっている
  （§15.6 当時は 67.6 / 73.7 ＝ +6.1 pt）。`GEMM_FUSED` が 966.9 → 940.3 µs に
  なったぶんで、**交差点は §15.6 が書いたときより Nq=256 側に寄っている**。
  過去の表は書き換えず、ここに注記として残す。
