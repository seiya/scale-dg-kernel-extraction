...
- **lift 31.4 µs**: 1 要素あたり 3 本の 16 バイトロードで、この因数分解の下限である。
  4 本の横面値は 1 ブロック内で再利用が無い（列タイル 64 は 1 つの `j` に収まるので
  `fb0[i+kNq]` はブロック内で全部相異なる）。体積サイズの lift 中間配列に戻す案は
  2026-08-25 に測って遅いことが分かっている。x / y GEMM 側へ移す案は、
  そちらが CUTLASS 標準 epilogue を手放すことになり §10.4 の +72 µs を払う。
