...
> **本節の「Nq=16 / 32 / 64 では 3 つとも負ける」は 16 B epilogue については
> 成立しない**: 担い手を x に移した上で測り直すと Nq=16 −2.56%、Nq=32 −7.28%、
> Nq=64（z 担い手のまま）−0.95%。とくに「Nq=64 の `GemmZWide` は +2.8%」は
> **16 B lift が同時に on だったときの値**で、off にすると符号が変わる。
> 2 つは相互作用しており、どちらも単独の判定ではない。
> 逆に、**本節が p=7 で採用した 16 B lift は担い手が x に移った時点で成立しなく
> なっている**: 採用形 pkg4 の上で測り直すと 4 次数すべてで +2.1〜3.2% 負ける
> （レジスタ 124 → 154 で Nq≤32 は SM あたり 1 CTA を失う）。現行の採用形に
> 16 B lift はどの次数でも入っていない。
...
## 14. CUDA graph 再生（2026-09-01、−1.44%）

p=7 / 15 / 31 をまとめて測った。記録は
[`p31_gap_study.md`](p31_gap_study.md) §29（job `75947`、親 `c386c85`、
12 回交互、レンジ非重複、`dqdt` ビット一致）。

p=7 は **`Step loop per stage` 1.79704e-3 → 1.77117e-3 s（−1.44%）**。
絶対の得は 25.9 µs/stage で 3 次数のなかで最大だが、率は最小である
（公開 conf の `Ne=32^3` で 1 ステージが 1797 µs と最も長いため）。
**既定 conf は graph off のまま**（graph 再生では `Cal_tend` /
