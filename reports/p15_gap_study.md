...
ncu は Slurm job `54662`（`job_ncu_p15.sh`、CUDA core 版）と
...
|---|---|---|
| `CUDAFORTRAN_FUSED_TC` | **0.02163** | **0.02756** |
...
| p=15 `FUSED_TC` | 0.022220 | 370.3 µs |
...
| **p=15 `FUSED_TC`** | **370.3 µs** | **5.61** | **14.0%** | **3.68** | **47%** |
...
- 残っている余地は memory pipe 67% と DRAM 32% の差、すなわち面 gather である。
  p=7 では ±x 面の M 側を shared に置いて −5.1% を得た
  （`tc_paper_survey_2407.09621.md` §13）が、Nq=16 では
  レジスタに余裕がゼロ（1024 スレッドでの上限 64 本に張り付き）なので、
  同じ手をそのまま持ち込むことはできない。
