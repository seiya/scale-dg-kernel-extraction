...
- **この計数は p=31 y カーネルの st コンフリクト 16〜21% と同じ族**である
  ...
  > **訂正注記（2026-09-03、§24.6）**: この 2 行の「**同族**」は正しいが、
  > 「**128-bit shared ストアに固有**」という幅による性格づけは誤りである。
  > レーン線形な 128-bit shared ストアは **4.000 wavefront/要求・コンフリクト 0**
  > を出す（§24.2）。共通項は幅ではなく **L1TEX Data-Stage の競合**である。
...
> **訂正注記（2026-09-03、§24）**: 本項の未解明は **§24 で閉じた**（job `78531`、
> node c187）。三点が変わる。(1) 「64-bit ストアの最小充当 2 wavefront/要求」
> という見積もり**そのものは正しい**（合成マイクロベンチでレーン線形の 64-bit
> ストアはちょうど 2.000、コンフリクト 0）。(2) **間違っていたのは要求数のほう**
> で、下の 80 / 144 要求/ブロックは実測では **72 / 136**（`smsp__inst_executed_op_shared_st.sum`。
> 差の 64 は正しく、両版とも一様に 8 だけ多く数えていた）。したがって
> **2.82 / 2.85 は 3.03 / 3.16 が正しく、超過は 0.82 ではなく 1.03** である。
> (3) **超過の出所は L1TEX Data-Stage の競合**である。このカウンタは
> 「Data-Stage が処理した wavefront 数」＝データ段のサイクル計数であって
> アドレス由来の量ではなく、**同一アドレス**のレーン線形 64-bit ストアに
> global ロードを重ねるだけで 2.000/0 が 2.150/39,348 になる。実カーネルでは
> 超過が占有率と global トラフィックの 2 変数で分離し、ワープ内ストアアドレスが
> ビット単位で同一の `KP=1` → `KP=2` で **1.032 → 0.248** に落ちる。
> 下の 3 点（3 群でそろう、アドレスの関数でない、時間の関数でない）は
> **すべて正しかった**。以下は当時の記述として残す。
...
## 21. p=7 `FUSED_TC` のチャンク数は 1（2026-09-03、符号則は棄却を予測、測定不要）

`TODO.md` §5 の 3 件目。本サーベイはチャンクループについて何も書いておらず、
[`p63_gap_study.md`](p63_gap_study.md) §52.3 の**符号則**——「チャンクループの
二重バッファが効くかはチャンク数で決まる」——を p=7 に当てられなかった。
ソースを読んでチャンク数を確定させる。

### 21.1 符号則（既知、再掲）

二重バッファはチャンクあたりのバリアを 2 本から 1 本に減らす手なので、
**チャンクが 1 個しか無い構成に持ち込むと、消せるバリアより増えるバリアの
方が多い**（[`p63_gap_study.md`](p63_gap_study.md) §52.3）。

| 次数 | 縮約チャンク数 | 二重バッファ | 判定 | 出所 |
|---|---:|---:|---|---|
| p=31 | 1 | +1.04〜+12.05% | 棄却 | [`p31_gap_study.md`](p31_gap_study.md) §30 |
| p=63 | 1 | +2.8〜3.1% | 棄却 | `p63_gap_study.md` §52.3 |
| p=127 | 2 | −4.25%（2×4 タイルと組で） | 採用 | [`p127_gap_study.md`](p127_gap_study.md) §23 |
| p=255 | 16 | −27.7%（4×4 タイルと組で） | 採用 | [`p255_gap_study.md`](p255_gap_study.md) §4.1 |

ここで「チャンク」は**縮約方向のパネル分割**（`BK63` / `BKD127` / `BK255`）
であって、j 平面のバッチループではない（`p31_gap_study.md` §30 が
その取り違えを訂正している）。

### 21.2 p=7 の事実: **チャンクループが存在しない**

`cuda_dg_kernels_tc.cu` の `tendency_fused_p7_kernel`:

- 3 方向の流束パネル `sFluxX` / `sFluxY` / `sFluxZ`（各 512 double）と
  `sflux_bnd` / `sMface` / `sDfrag` / `sLift` が**全部 shared に常駐**する。
  ptxas は静的 shared **20,864 B**、レジスタ **32**、spill 0
  （`-arch=sm_100`、`__launch_bounds__(256, 8)`）。
- 縮約長は `Nq = 8`。これは `mma_k8_8x8()` の中で
  **`mma.sync.m8n8k4` 2 本（k0 = 0 と k0 = 4）を直に並べる**形で消化される。
  **ループが 1 本も無い。** `for` も `#pragma unroll` も、
  global → shared のステージングを回す構造も存在しない。

したがって p=7 の**縮約チャンク数は 1**であり、より正確には
**パイプライン化する相手（ステージングのループ）が 1 反復も無い**。
p=63 / p=31 と同じ側で、符号則は**棄却**を予測する。

### 21.3 チャンクを作る側にも余地が無い（占有率の数え上げ）

p=63 §52.3 は「二重バッファを測るには `BK63` を下げてチャンクを作る必要がある」
として実際にそれをやり、負けた。p=7 で同じことをするなら shared を減らして
ブロックを増やすのが動機になるが、**その動機が無い**。

| 量 | p=7 `FUSED_TC` |
|---|---:|
| スレッド / ブロック | 256 |
| `__launch_bounds__` の 2 引数目 | **8** |
| shared / ブロック | 20,864 B |
| shared 8 枚ぶん | 166,912 B ≤ 233,472 B（**余っている**） |
| レジスタ | 32 |
| レジスタ 8 枚ぶん | `32 · 256 · 8` = **65,536 = レジスタファイル全部** |
| 占有率 | `8 · 256 / 2048` = **100%** |

**占有率は既に 100% で、律速はレジスタファイルの飽和である。** shared を
半分にしてもブロックは 1 枚も増えない。チャンクを作って得られるのは
バリアの増加だけで、符号は確定して負である。

### 21.4 判断

**棄却。実装しない。** 根拠は「効果が小さそうだから」ではなく、
(a) 二重バッファの対象であるチャンクループが**存在しない**（§21.2）、
(b) チャンクを新設する側の動機である占有率が**既に 100%**（§21.3）、
の 2 つの数え上げである。`AGENTS.md` の終了条件「天井が測定誤差以下」を
数え上げで満たしている。

**副産物**: 同じ数え上げが、`P7_BPSM` を上げすぎると壊れる
（[`p7_gap_study.md`](p7_gap_study.md) §5 の副産物、`minBlocks` 7・8 が
スピル 0 のまま +44%）ことと表裏である。p=7 TC は
`P7_BPSM = 8` で shared・レジスタとも**ちょうど**使い切っており、
どちらの資源にも遊びが無い。

## 22. p=7 への PDL 適用可否（2026-09-03、適用箇所が無い。天井は graph 再生で実測）
...
[`p31_gap_study.md`](p31_gap_study.md) §25 と 3 次数に横展開されたが、
p=7 / p=15 では一度も言及が無かった。本節が p=7 の記録である
（p=15 は [`p15_gap_study.md`](p15_gap_study.md) §32）。

commit は本節を追加したもの（親 `709334a`）、GPU は RIKYU GB200、
入力は `namelists/perf_p7_fused_tc.conf`（`Ne=32³`、`nstep=20`、graph off）、
`nsys` は Slurm job **`78241`**（node `c182`、凍結 exe
`scale-dg_extraction.pdl_probe`、`--trace=cuda --sample=none
--resolve-symbols=false`、`DEBUGINFOD_URLS=` 済み）。graph の A/B は login GPU の
5-run 中央値。

### 22.1 PDL の適用条件（§19 / §50 / §25 から）

PDL は「先行グリッドが `griddepcontrol.launch_dependents` を出した時点で、
`cudaLaunchAttributeProgrammaticStreamSerialization` を付けて起動された後続
グリッドのブロックが立ち上がってよい」という仕組みである。成立には
**同一ストリーム上に連続する 2 本の CUDA C++ グリッド**が要る。実績:

| 段 | 内容 | 効果 |
|---|---|---:|
| 面 → xz（弱 PDL、暗黙ヒント） | p=63 s1 | **0**（レンジ重複） |
| 面 → xz（面の先頭で `launch_dependents`） | p=63 s2 | −0.5% |
| **xz → y**（y を xz の従属に） | p=127 §19 / p=63 s3 / p=31 §25 | **−2.71% / −3.31% / −0.76%** |

**一次グリッドが Fortran（OpenACC）だと重ならない**ことも測ってある
（`p127_gap_study.md` §19.2「Fortran 面のまま xz だけ PDL」はレンジ重複で棄却）。

### 22.2 p=7 の事実: **tendency は 1 グリッドである**

`launch_tendency_fused_p7_impl` は
`tendency_fused_p7_kernel<UseTc><<<Ne, P7_THREADS>>>` を**1 本だけ**起動する。
面フラックス・x/y/z の 3 微分・エピローグが全部この 1 カーネルの中にある
（CUDA-core 側 `tendency_fused_p7_cc_kernel` も同じく 1 本）。
**p=127 の梯子の段が 1 つも存在しない。**

nsys の GPU trace（job `78241`）が示す 1 stage の全内容:

| グリッド | 生成元 | 中央値 [µs] |
|---|---|---:|
| `main_rk_update_stage_462_gpu`（stage 1 は `_455_`） | OpenACC | 74.21（76.66） |
| `mod_mesh_update_halo_243_gpu` | OpenACC | 4.86 |
| `tendency_fused_p7_kernel<true>` | **CUDA C++** | **264.18** |
| カーネル間の隙間 合計 | — | **9.81** |
| **stage 周期** | | **355.20** |

（stage 周期 355.20 µs は login の `Step loop per stage` 355.24 µs と
0.01% で一致する。）

CUDA C++ のグリッドは 1 本しかなく、その前後の隣は**どちらも OpenACC が
生成したローンチ**である。`cudaLaunchAttributeProgrammaticStreamSerialization`
を付ける先が無く、§22.1 の実測どおり一次側が Fortran なら重ならない。
**PDL は p=7 では書けない。**

### 22.3 それでも取りにいく相手（隙間）の天井を測った: **2.8%、しかも既に取れる**

PDL が p=7 で狙いうる唯一の量は、上表の**隙間 9.81 µs = stage の 2.76%**である。
この天井は `UseCudaGraph = .true.` で直接測れる（graph 再生はホスト側の
ローンチ待ちを消すので、隙間の除去としては PDL の上位互換）。

| | 中央値 [µs/stage] | レンジ | 差 |
|---|---:|---|---:|
| graph off（既定） | **355.24** | 354.53–355.88 | — |
| **graph on** | **345.13** | 344.69–345.72 | **−2.85%（−10.11 µs）** |

login GPU 5-run、レンジ非重複。**取れた 10.11 µs は nsys が測った隙間
9.81 µs の 103%** で、隙間はまるごとホスト側のローンチ待ちであり、
graph 再生がそれを全部回収していることが確定する。

したがって p=7 における「ローンチ側の天井」は **2.8% で、しかも
`UseCudaGraph = .true.` で既に回収できる**。PDL がその上に足せる分は
**0** である。

### 22.4 仮に段を作っても勝てない（tail の数え上げ）

参考までに、カーネルを分割して段を作る案も数え上げで閉じておく。
PDL が回収するのは先行グリッドの**排水（tail）**であって隙間ではない
（`p63_gap_study.md` §50.5、`p127_gap_study.md` §20.1）。p=7 は
`Ne = 32³ = 32,768` ブロック、`__launch_bounds__(256, 8)`、SM 152 枚なので
同時常駐 1,216 ブロック、**27.0 wave** 回る。tail は最良でもカーネルの
`1/27 = 3.7%`、そのうち PDL が重ねられるのは後続の立ち上がりぶんだけである。
p=127 の y PDL が −2.71% と大きかったのは、あちらの xz が
`Ne = 2³` で wave 数が小さく tail が相対的に厚いからで、
**p=7 はその逆側にいる**。加えて分割はカーネル 1 本ぶんの
global 往復（融合が消したもの）を戻すので、符号は負である。

### 22.5 判断

**適用不能。実装しない。** 記録として:
(a) tendency は 1 グリッドで PDL の段が存在しない（§22.2）、
(b) 隣接する 2 本は OpenACC 生成で属性を付けられず、一次が Fortran では
重ならないことは p=127 §19.2 で実測済み、
(c) PDL が狙える隙間の天井は **2.76%（9.81 µs/stage）**で、
`UseCudaGraph = .true.` が **−2.85%** でそれを全額回収している（§22.3）。

**既定 conf は graph off のまま**とする（graph 再生では `Cal_tend` /
`CUDA device *` が出ず `nsys` も当てられない。
[`README.md`](README.md) の p=7/15/31 `GEMM_FUSED` の graph 記録と同じ判断）。
なお `GEMM_FUSED` 側の graph 利得は p=7 で 22〜26 µs/stage だったのに対し
`FUSED_TC` は 10 µs である。1 stage のカーネル本数が
`GEMM_FUSED` の 6 本前後に対し **3 本**しか無いからで、
「得は µs でほぼ一定、率は stage の長さで決まる」という README の見立ての、
本数側からの裏づけになる。
...
## 23. p=7 `FUSED_TC` の出力タイル (A) と、この経路の探索終了宣言（2026-09-03、採用 −7.15%）

`TODO.md` §6 の最後の 2 行。ひとつは「**p=7 `FUSED_TC` には探索終了宣言が無い**
（結論が §6/§7/§13/§15/§16 に分散している）」、もうひとつは §19.10 が
「**p=7 TC への出力タイル軸 (A) の適用は別項目として未着手**」と明記して
残した宿題である。本節は後者を測ってから前者を書く。**(A) は p=7 TC でも
勝った（−7.15%）ので、宣言はこの改修後の形について出す。**

### 23.1 測定条件と物差し

- 親 commit `219e78d`（`feature/cuda`）＋本節の作業ツリー、GB200 1 GPU、
  `make CUDA=1 GPUFLAGS=-gpu=cc100`
- 入力 `namelists/perf_p7_fused_tc.conf`（`Ne=32³`、`PolyOrder=7`、`nstep=20`、
  `UseCudaGraph=.false.`）。`FUSED_DFMA` は `DqdtKernel_Type` だけ差し替えた
  作業コピー。`NeX/NeY/NeZ`・`dt`・`nstep` は全比較で固定
- 物差しは **`CUDA device fused tendency` ÷ (19 steps × 3 RK) = ÷57**
  （§18・`p7_gap_study.md` §5 と同じ量）
- 掃引は login node の 3-run 中央値。**採否は Slurm で占有した GPU 上の
  12 ラウンド交互 A/B**（job **`78266`**、node **c384`**）。経路横断の
  最終値は job **`78271`**、node **c177**、12 ラウンド
- ncu は job **`78272`**、node c177、メトリクス指定、`nstep=4`、
  `-k regex:tendency_fused_p7 -s 4 -c 2`、改修前後を同一ジョブに。
  **ncu の時間は採否に使っていない**（クロック固定バイアス）

### 23.2 (A) を p=7 TC でどう書くか —— 1 ワープが持つ k 平面の数

p=7 TC の 1 ブロックは 1 要素（512 ノード）を 256 スレッドで覆い、
**ワープ = k 平面**である（`k = warp`、8 ワープで 8 平面）。`m8n8k4` の
アキュムレータは 1 レーンあたり 2 要素なので、**1 スレッドの出力は 2**
——`p7_gap_study.md` §5 が CC 側で 2 → 4 にした、あの位置にいる。

(A) の適用は「1 ワープが `KP` 枚の k 平面を持つ」ことである。ノブ
`P7_TC_KP`（`fused_kernel_geom.h`）を足し、`P7_THREADS = 256 / KP`、
`P7_NWARP = P7_THREADS / 32` とした。`KP = 2` なら 128 スレッド・4 ワープ、
ワープ `w` は `k = w` と `k = w + 4` を持ち、**1 スレッド 4 出力**になる。

書き換えは 4 か所で、いずれも「ワープ番号を持ち回る量」を `KP` 倍にするだけ:
体積フラックスの充填（1 スレッド `2*KP` ノード）、面フラックス（384 面点を
`THREADS` ずつ）、z 縮約（ワープが `KP` 個の列グループを持つ。ストアが読み元の
`sFluxZ` を潰すので `KP` 個の結果をバリアの前にレジスタへ揃える）、
x/y と エピローグ（`kk` ループ、`n0 = 64*k + 2*lane`）。

**production のカーネルは 1 行も触っていない。** `tendency_fused_p7_kernel` は
32 レジスタ・8 ブロック/SM でレジスタファイルをちょうど使い切っており
（§21.3）、上の一般化を `KP` のテンプレートとして**同じ関数に**入れると、
`KP = 1` でも ptxas が 8 バイトの spill を出す（体積ループで 4 B、`kk` ループで
4 B、それぞれ単独でも出る）。そこで一般形は
**`tendency_fused_p7_tile_kernel` という別関数**にし、`P7_TC_KP == 1` の
ときだけ元の関数を起動する `#if` をローンチ側に置いた。**`KP = 1` ビルドの
オブジェクトは HEAD と SASS 完全一致**である（`cuobjdump -sass` の差分 0 行）。

### 23.3 はしご（login 3-run 中央値、µs/stage = device ÷ 57）

| 形 | スレッド | 出力/スレッド | REG / spill | ブロック/SM | 占有率 | µs/stage | 対 production |
|---|---:|---:|---:|---:|---:|---:|---:|
| **`KP=1`（従来 production）** | 256 | 2 | 32 / 0 | 8 | 100% | 273.3 | — |
| `KP=2`, `P7_BPSM=8` | 128 | 4 | 64 / 0 | 8 | 50% | 291.2 | +6.6% |
| `KP=2`, `P7_BPSM=7` | 128 | 4 | 72 / 0 | 7 | 43.8% | 259.1 | −5.2% |
| **`KP=2`, `P7_BPSM=6`（採用）** | 128 | 4 | 80 / 0 | 6 | 37.5% | **254.7** | **−6.8%** |
| `KP=2`, `P7_BPSM=5` | 128 | 4 | 92 / 0 | 5 | 31.3% | 271.9 | −0.5% |
| `KP=2`, `P7_BPSM=4` | 128 | 4 | 92 / 0 | 4 | 25% | 276.6 | +1.2% |
| `KP=4`, `P7_BPSM=8` | 64 | 8 | 128 / 0 | 8 | 25% | 310.5 | +13.6% |
| `KP=4`, `P7_BPSM=6/5/4/3/2` | 64 | 8 | 142–144 / 0 | — | — | 301.2–302.5 | +10.3〜10.6% |
| `KP=8`, `P7_BPSM=4` | 32 | 16 | 212 / 0 | 4 | 6.3% | 398.0 | +45.6% |

**(B) の符号則がここでも両側に出る。** 同じ `KP=2` のタイルが、8 ブロックの
予算（64 レジスタ）では **+6.6%**、6 ブロック（80 レジスタ）では **−6.8%**
である。spill は全形で 0 なので、これは「spill させない」ではなく
「**タイルには予算が付いてこなければならない**」側の例で、
`p7_gap_study.md` §5・`p255_gap_study.md` §26.6 に続く 4 例目になる。
**梯子は `KP=4` で終わる**（1 スレッド 8 出力は、予算を 2〜8 ブロックの
どこに置いても +10% 以上）。

**記録すべき失敗**: `KP=4` は最初 **195 µs（−28.6%）** と出た。オペレータの
shared 充填が `if (tid < 64) … else if (tid < 112)` と書かれていて、
**64 スレッドでは `sLift` が一度も埋まらない**ためである（lift 項が消えた
ぶんだけ速い壊れたカーネル）。`THR` ずつ回る形に直すと 302 µs になる。
その一方、同じ「THR ずつ回る形」を `KP=2` にも使うと 254.7 → 269.6 と
**4% 落ちる**ので、112 スレッド以上では元の一発形を `if constexpr` で
残してある。掃引の途中で出た「大きすぎる勝ち」は、まず**カバレッジの
数え上げ**を疑うべきだという記録である。

### 23.4 採否（job `78266`、node `c384`、12 ラウンド交互、レンジ非重複）

| 変種 | 中央値 µs/stage | レンジ | 対 `KP=1` |
|---|---:|---:|---:|
| `FUSED_TC` `KP=1`（従来） | 273.71 | 272.92–274.27 | — |
| **`FUSED_TC` `KP=2`/`BPSM=6`（採用）** | **254.15** | 253.66–254.96 | **−7.15%** |
| `FUSED_TC` `KP=2`/`BPSM=7` | 259.19 | 258.57–259.53 | −5.30% |
| `FUSED_DFMA` `KP=1`（従来） | 424.38 | 424.01–424.70 | — |
| **`FUSED_DFMA` `KP=2`/`BPSM=6`** | **334.58** | 334.43–334.84 | **−21.16%** |

`FUSED_DFMA` は同一ソースの `UseTc=false` なので、**変更は両方に同時に入る**
（`AGENTS.md`）。iso-schedule は保たれている: `UseTc` を見る分岐は
`mma_m8n8k4_f64` の 1 か所だけで、`P7_TC_KP` はそこを見ない。

DFMA が TC より大きく（−21.2% 対 −7.2%）動くのは、DFMA 側が `KP=1` の
32 レジスタ予算で **84 B の spill** を出していた（§26.2 が p=255 で見つけた
のと同じ現象）ためで、`KP=2`/80 レジスタでは spill 0 になる。

### 23.5 機構（ncu job `78272`、`Ne=32³`、改修前後を同一ジョブに）

| メトリクス | `KP=1` | `KP=2`/`BPSM=6` | 変化 |
|---|---:|---:|---:|
| shared wavefront | 2.947e7 | 2.331e7 | **−20.9%** |
| 発行命令 | 8.110e7 | 6.721e7 | **−17.1%** |
| shared バンクコンフリクト（st） | 4.117e6 | 6.237e5 | **−84.9%** |
| shared バンクコンフリクト（ld） | 3.712e5 | 3.570e4 | **−90.4%** |
| global sector（LD） | 7.864e7 | **7.864e7** | **±0** |
| global request（LD） | 7.864e6 | **7.864e6** | **±0** |
| DRAM read / write | 1451 / 128.7 MB | 1447 / 127.9 MB | −0.3% / −0.7% |
| `barrier` | 2236 | 523.2 | **−76.6%** |
| `long_scoreboard` | 7393 | 4132 | −44.1% |
| `short_scoreboard` | 1961 | 1036 | −47.2% |
| `mio_throttle` | 1183 | 1894 | **+60.1%** |
| `wait` | 737.4 | 959.4 | +30.1% |
| L1/TEX | 91.09% | 91.84% | +0.8% |
| 達成占有率 | 96.4% | 35.9% | −62.8% |
| ncu duration | 387.5 µs | 356.5 µs | −8.0% |

`p63_gap_study.md` §53.7 / `p127_gap_study.md` §24.7 / `p255_gap_study.md` §26.7 と
**同じ署名**である: shared wavefront と命令が出力数で割られ、
**global セクタとリクエストは 1 本も動かず、DRAM バイトも動かない**。
p=7 で新しいのは 2 点:

1. **shared wavefront が 20.9% 減る。** CC 側（`p7_gap_study.md` §5）では
   −6.4% しか動かなかった。TC 側は z 微分が shared を往復し、x/y の
   オペランドも shared から来るので、(A) が割る相手が CC より多い。
   `p15_gap_study.md` §27.3 の判定基準（(A) の**大きさ**は shared wavefront の
   詰まり方で決まる）が、**同じ次数の 2 経路の間で**成立している例になる。
2. **§13.5 / §20 が閉じられなかった shared store コンフリクトが 85% 消えた。**
   §20.4 は「アドレスをいじってコンフリクトを減らしても時間は動かない
   （天井は負）」と結論していたが、出力タイルは**コンフリクトを減らす目的で
   触っていないのに**、1 スレッドが持つ面点・ノードが倍になった結果として
   消している。§20.5 が「まだ説明できていない」と書いた分の一部はこれで
   説明が付く: コンフリクトは律速ではなかったが、**それを作っていた
   スレッド配置そのものが律速だった**。

L1/TEX は 91% の屋根に残り、`mio_throttle` と `wait` は増えている。
p=7 は §0 のとおり global ロード待ちの次数で、(A) はその律速を動かさない。
動いたのは**隠す側**（命令・バリア・shared）である。

### 23.6 数値検証

`SCALE_DG_VARYING_COEFF=1`、`namelists/val_p7_split.conf` 対照（`CUDAFORTRAN_SPLIT`）、
`Ne=2³` の owned `dqdt` 全 4,096 点。

| 比較 | max abs | 相対 |
|---|---:|---:|
| `FUSED_TC`（採用形）対 `SPLIT` | **1.77636e-15** | 2.86e-16 |
| `FUSED_DFMA`（採用形）対 `SPLIT` | **1.77636e-15** | 2.86e-16 |
| `FUSED`（CC、未変更）対 `SPLIT` | 1.77636e-15 | 2.86e-16 |
| `FUSED_TC` 対 `FUSED_DFMA` | **全点ビット一致** | — |

§5.4 / §18 と同じ値である。`KP=1` ビルドは HEAD と SASS 完全一致なので、
従来形の数値も変わっていない。Fortran 側のインタフェースは変えていない
（ノブは `.cu` と `fused_kernel_geom.h` の中だけ）ので、stub の同期は不要。

### 23.7 現在地と、3 つの比

job `78271`（c177、12 ラウンド交互、最終ツリー、3 経路同居）:

| 経路 | 中央値 µs/stage | レンジ | `Main` [ms/step] |
|---|---:|---:|---:|
| `CUDAFORTRAN_FUSED_TC` | **253.76** | 253.57–254.02 | 1.0082 |
| `CUDAFORTRAN_FUSED_DFMA` | **335.35** | 335.06–336.24 | 1.2490 |
| `CUDAFORTRAN_FUSED`（CC） | **292.83** | 292.53–293.02 | 1.1236 |

レンジは互いに重ならない。

- **機構比 A（`FUSED_DFMA` / `FUSED_TC`）= 335.35 / 253.76 = 1.322×**
  （§18 の 1.551× から。分子分母とも本節で動いた）
- **主比 B（`FUSED` / `FUSED_TC`）= 292.83 / 253.76 = 1.154×**
  （§18 の 1.070× から。CC 側は未変更）
- 253.76 µs/stage は **5.54 TFLOP/s（ピーク 40.1 の 13.8%）**
  （FLOP 数は `p7_gap_study.md` §5.5 の 292.3 µs / 4.81 TFLOP/s を時間で
  割り直したもの。§23.5 のとおり DRAM バイトは動いていない）
- **最速経路は `CUDAFORTRAN_FUSED_TC` のまま**

### 23.8 p=7 `FUSED_TC` の探索終了宣言（`TODO.md` §6）

この経路の結論は §6・§7・§13・§15・§16・§19〜§22 と本節に分散していた。
**1 か所にまとめて、ここで探索を終了する。**

契約内で測って**採用**したもの（時系列）:

| 節 | 変更 | 効果 |
|---|---|---:|
| §6 | shared レイアウト刷新（コンフリクトフリー化、論文 A-1） | 採用 |
| §7 | shared 削減による占有率 100% 化、分離可能 lift | 採用 |
| §11 | 整数・アドレス演算の削減（版 III） | 採用 |
| §13 | ±x 面 M 側 gather の shared 経由（`sMface`） | 採用 |
| **§23** | **出力タイル (A) ＋ レジスタ予算 (B)** | **−7.15%** |

契約内で測って**落とした**もの、および天井が測定誤差以下と示したもの:

| 出所 | 候補 | 結果・機構 |
|---|---|---|
| §6.3 / §7.4 / §10 | shared レイアウトの別置換（マイクロベンチ上は最適） | 実機で遅い。§10.4 が機構 |
| §9 | M 側 gather を shared に移す（全面） | 不採用。§13 が ±x 面だけ採った |
| §12 | face gather の前倒し | 不採用。依存の並びは律速でない |
| §14 | D1D フラグメントのレジスタ常駐 | 不採用。32 レジスタ予算に余白が無い（**§23 で予算が 80 に増えたので、これは再点検の対象になる** —— 下記） |
| §16 | z の shared 往復を消す | 天井そのものが小さい |
| §19 | `m16n8k*` FP64 mma（役割外） | sm_100 に命令が無い。払える形で +24% |
| §20 | shared store コンフリクトの除去 | 天井は負。**§23.5 で結果的に 85% 消えた** |
| §21 | チャンクループ二重バッファ | チャンクループが存在しない（数え上げ） |
| §22 | PDL | 段が存在しない。隙間 2.76% は graph が回収済み |
| §23.3 | `KP=4` / `KP=8`、`KP=2` の予算 8/7/5/4 | 上表のとおり全部負け |

**終了の根拠は「効果が小さそうだから」ではない。** 残っている契約内の量は
§0 以来の **global ロード待ち（L1/TEX 91%、`long_scoreboard` が最大の stall）**
であり、(A) はそれを 1 セクタも動かさない（§23.5）。セクタを減らす候補は
§9・§12・§13 で測り尽くし、採れたのは §13 だけである。

**本節が前提を壊した候補は 1 つだけあり、それも測って落とした**（§23.9）。
`AGENTS.md` の「覆った宣言はいずれも候補集合が古かった」に自分で従うため、
宣言を出す前にそこを閉じてある。それ以外は上表のとおり、機構つきで閉じている。

### 23.9 §14（D1D フラグメントのレジスタ常駐）の再点検 —— 前提は変わったが、やはり負け

§14 が落ちた理由は「**32 レジスタ予算に余白が無く、ptxas がスピルする**」
だった。§23 の採用形は予算が **80** で、しかも x/y の mma 対を `KP` 回
発行するので、消せる再ロードは 2 本ではなく `2*KP` 本のはずである。
**前提が変わったので測り直した**（ノブ `P7_TC_DREG`、既定 0）。

| | レジスタ | spill | SASS `LDS` | SASS 総命令 | login µs/stage（3-run 中央値） |
|---|---:|---:|---:|---:|---:|
| 既定（`sDfrag` から都度） | 80 | 0 | 44 | 536 | **254.59** |
| `P7_TC_DREG=1` | 80 | 0 | **42** | **536** | 268.05（**+5.3%**） |

**スピルは消えたのに、やはり負ける。** `LDS` は 2 本しか減らない
（§14.2 のとおり ptxas は既に x と y の再ロードを共有しており、`KP=2` でも
その共有が効いている）、総命令数は 1 本も動かない、そして 2 本の shared
ロードをレジスタで買うために `sDz` の 2 本のバリアを跨いで生存区間が伸びる。
login の 3-run はレンジ幅 0.5% 級で、差の 5.3% はその 10 倍である。

これは §14.5 の結論の**強い形**である: §14 は「買うための余白が無い」と
書いたが、**余白があっても買えない**。§10.4（メモリ命令 1 本をアドレス演算
2 本で買う取引は成立しない）と同じ壁が、レジスタ側にも立っている。
ノブは既定 0 でツリーに残した。

---

## 24. §20.5 の「1 要求あたり 2.8 wavefront」の出所（2026-09-03、機構特定）

`TODO.md` §4 の 1 行目が残していた最後の未解明である。§20 は
「2.41 M → 4.10 M の増分」と「その時間への寄与」を閉じたが、
**1 要求あたりの実 wavefront が 2 ではなく 2.8 であることの出所**を
「アドレスの関数でも時間の関数でもない」までしか詰めていなかった。

**結論を先に書く。3 つある。**

1. **「64-bit shared ストアの最小充当は 1 要求 2 wavefront」という見積もりは
   正しい。** 合成マイクロベンチでレーン線形の 64-bit ストアは
   **ちょうど 2.000 wavefront/要求・コンフリクト 0** を出す（32-bit は 1.000、
   128-bit は 4.000）。見積もりのほうは間違っていなかった。
2. **間違っていたのは要求数のほうである。** §20.2 は 1 ブロックあたりの
   ストア要求を SASS から手で 80 / 144 と数えたが、実測は **72 / 136** で、
   どちらも 8 だけ多く数えていた（差の 64 は正しい）。したがって
   正しい値は 2.82 / 2.85 ではなく **3.03 / 3.16** で、**未説明の超過は
   0.82 ではなく 1.03 wavefront/要求**である。穴は塞がるどころか広がった。
3. **超過の出所は L1TEX Data-Stage の競合であって、アドレスではない。**
   このカウンタは「Data-Stage が処理した shared wavefront 数」、すなわち
   **ユニットのサイクル計数**であり、アドレス由来の量ではない。
   **同一アドレス**のレーン線形 64-bit ストアに global ロードを重ねるだけで、
   計数は 2.000 / コンフリクト 0 から **2.150 / コンフリクト 39,348** に動く。
   実カーネル側では、超過は**同時実行ワープ数と global トラフィック量の
   2 変数で分離して動き**、ワープ内ストアアドレスが 1 ビットも変わらない
   `KP=1` → `KP=2` で **1.03 → 0.25** に落ちる。

| | |
|---|---|
| commit | `44b02a6`（`feature/cuda`）。作業ツリーの変更は本節のマイクロベンチとビルドスクリプトのみで、**カーネルソースは 1 行も触っていない** |
| ビルド | `module load nvhpc-hpcx`（nvhpc 26.3、nvcc 13.1）、`make CUDA=1 GPUFLAGS=-gpu=cc100`。変種は `scripts/build_p7tc_variant.sh <tag> [-D...]` で `cuda_dg_kernels_tc.cu` だけ再コンパイルして再リンク |
| 入力 | `namelists/perf_p7_fused_tc.conf`（`Ne=32³`、`PolyOrder=7`、`nstep=20`、graph off）の `nstep=4` コピー `output/ncu_p7_u4.conf`。`NeX/NeY/NeZ`・`PolyOrder`・`dt` は不変 |
| 測定 | Slurm job **`78531`**、node **`c187`**、GB200 1 GPU。マイクロベンチと実カーネル 4 版を**同一ジョブ**に入れた |
| マイクロベンチ | `scripts/micro_shared_wavefront.cu`（`nvcc -O3 -std=c++17 -arch=sm_100`）。11 パターン、各カーネルちょうど 8 STS/スレッド × 8 ワープ × 4096 ブロック = **262,144 要求**を構成上保証 |
| ジョブスクリプト | `jobs/job_p7_shwf.sh`（`jobs/` は gitignore なので、ncu コマンドは下に転記した） |

`ncu` コマンド（`--set basic` ではコンフリクトが採れないので明示指定）:

```
MET=l1tex__data_bank_conflicts_pipe_lsu_mem_shared_op_{st,ld}.sum,\
l1tex__data_pipe_lsu_wavefronts_mem_shared_op_{st,ld}.sum,\
l1tex__data_pipe_lsu_wavefronts_mem_shared.sum,\
l1tex__t_requests_pipe_lsu_mem_shared_op_{st,ld}.sum,\
smsp__inst_executed_op_shared_{st,ld}.sum,smsp__inst_executed.sum,\
l1tex__t_{sectors,requests}_pipe_lsu_mem_global_op_ld.sum,\
l1tex__throughput.avg.pct_of_peak_sustained_elapsed,\
sm__throughput.avg.pct_of_peak_sustained_elapsed,\
sm__warps_active.avg.pct_of_peak_sustained_active,gpu__time_duration.sum
...
# マイクロベンチ（11 カーネル）
timeout 900 ncu --csv --metrics "$MET" --kernel-name-base function \
  --rename-kernels 0 -k regex:k_d -s 11 -c 11 ./micro_shared_wavefront

# 実カーネル 4 版
for tag in kp2 kp1 nostage kp1_nostage ; do
  timeout 900 ncu --csv --metrics "$MET" --kernel-name-base function \
    --rename-kernels 0 -k regex:tendency_fused_p7 -s 6 -c 3 \
    ./scale-dg_extraction.$tag output/ncu_p7_u4.conf
done
```

`kp1` = `-DP7_TC_KP=1`（§20 が測った旧形）、`kp2` = 既定（§23 の現行 production 形）、
`nostage` = `-DP7_ABL_NOSTAGE=1`、`kp1_nostage` = 両方。

### 24.1 要求数はカウンタで採る —— `l1tex__t_requests_...op_st` は GB100 で `n/a`

まずカウンタ側の足場を作る。§20 が要求数を手で数えたのは
`l1tex__t_requests_pipe_lsu_mem_shared_op_st.sum` を使わなかったからだが、
**このメトリクスは GB100 では値を返さない**（4 版すべてで `n/a`。
shared は L1TEX の tag ステージを通らないので、t-stage のリクエスト計数に
乗らない）。使えるのは **`smsp__inst_executed_op_shared_st.sum`** で、
マイクロベンチではこれが構成上の要求数 262,144 と**全 11 カーネルで一致**した。
以後、要求数はすべてこのカウンタである。

`ncu --query-metrics` が返す定義（job `78531`、`output/shwf_querymetrics.txt`）:

| メトリクス | 定義 |
|---|---|
| `l1tex__data_pipe_lsu_wavefronts_mem_shared` | *# of shared memory wavefronts processed by **Data-Stage** for LDS, LD, 3D* |
| `l1tex__data_bank_conflicts_pipe_lsu_mem_shared_op_st` | *# of shared memory data bank conflicts generated by STS, ST, 3D* |

前者は「**Data-Stage が処理した**」数、すなわち L1TEX のデータ段が使った
サイクルであって、アドレスから決まる理想値ではない。**この 1 行が本節の
答えの半分**である。§20 は「wavefront − 最小 wavefront」という関係式は
正しく読んでいたが、wavefront 側が純粋なアドレス関数だと暗黙に仮定していた。

### 24.2 マイクロベンチの対応表（job `78531`、`scripts/micro_shared_wavefront.cu`）

各カーネルは 8 STS/スレッド・256 スレッド・4096 ブロックで、分岐も
ループも無く、SASS を `cuobjdump` で数えて 8 本ちょうどであることを
確認してある（幅も確認済み: 64-bit 版は `STS.64`、128-bit 版は `STS.128`、
32-bit 版は `STS`）。要求数はどの版も **262,144**。

| パターン | 幅 | 命令 | wavefront | コンフリクト | **wf/要求** | コンフリクト率 |
|---|---|---:|---:|---:|---:|---:|
| `d32_linear`（レーン線形） | 32 | 262,144 | 262,144 | 0 | **1.000** | 0.00% |
| `d32_stride2` | 32 | 262,144 | 524,288 | 262,144 | **2.000** | 50.00% |
| **`d64_linear`（レーン線形）** | 64 | 262,144 | 524,288 | **0** | **2.000** | **0.00%** |
| `d64_linear_8lane`（32 レーン中 8 本だけ活性） | 64 | 262,144 | 524,288 | 0 | **2.000** | 0.00% |
| `d64_xor`（XOR 置換、構成上コンフリクトフリー） | 64 | 262,144 | 524,288 | 0 | **2.000** | 0.00% |
| `d64_bcast`（ワープ全レーンが同一アドレス） | 64 | 262,144 | 524,288 | 0 | **2.000** | 0.00% |
| `d64_stride2` | 64 | 262,144 | 1,048,576 | 524,288 | **4.000** | 50.00% |
| `d64_stride4` | 64 | 262,144 | 2,097,152 | 1,572,864 | **8.000** | 75.00% |
| `d64_stride8`（±x 面の `Fmask` 歩幅） | 64 | 262,144 | 4,194,304 | 3,670,016 | **16.000** | 87.50% |
| `d128_linear`（レーン線形） | 128 | 262,144 | 1,048,576 | **0** | **4.000** | **0.00%** |
| **`d64_linear_gpressure`（`d64_linear` と同一アドレス＋global ロード）** | 64 | 262,144 | **563,636** | **39,348** | **2.150** | **6.98%** |

読み取れることが 4 つある。

1. **最小充当の見積もりは正しい。** 幅 32/64/128 bit のレーン線形ストアは
   ちょうど 1.000 / 2.000 / 4.000 で、コンフリクトはきっかり 0 である。
   §20 が置いた「64-bit は 2」は正しかった。
2. **均一なアドレスパターンからは 2 の冪の整数しか出ない**（2, 4, 8, 16）。
   **3.03 という非整数は、どの単一アドレスパターンにも対応しない。**
   混合か、アドレス以外の項のどちらかである。
3. **活性レーン数は効かない。** 32 レーン中 8 本だけが書く形（`stage_xface()`
   が `node & 7` で選ぶ形）でも 2.000 で、部分活性は計数を上げも下げもしない。
   ワープ全レーンが同一アドレスに書く形も 2.000 で、書き込み衝突は
   コンフリクトとして数えられない。
4. **決定的なのは最終行である。** `d64_linear_gpressure` は
   `d64_linear` と**アドレス式が同一**（`s[tid + t*256]`）で、幅も本数も
   活性レーンも同じで、違いは同じワープが global ロードを 8 本抱えていることだけ。
   それだけで **2.000 → 2.150**、コンフリクト **0 → 39,348** になる。
   **構成上コンフリクトフリーなアドレスに、アドレスと無関係な理由で
   「バンクコンフリクト」が計上される。**

### 24.3 実カーネル 4 版 —— 現行形 `KP=2` と旧形 `KP=1`（job `78531`、c187）

`tendency_fused_p7[_tile]_kernel<true>`、3 launch の平均。`Ne=32³` なので
1 launch = 32,768 ブロック。`kp1` / `kp1_nostage` は §20 が測った旧形
（`-DP7_TC_KP=1`）、`kp2` / `nostage` は §23 で採用した現行形。

| 版 | 形 | ステージング | ストア要求 | 要求/ブロック | wavefront | コンフリクト | **wf/要求** | **超過** | 率 |
|---|---|---|---:|---:|---:|---:|---:|---:|---:|
| `kp1_nostage` | `KP=1` | 無 | 2,359,296 | **72.0** | 7,464,760 | 2,254,648 | **3.164** | **1.164** | 30.20% |
| `kp1` | `KP=1` | 有 | 4,456,448 | **136.0** | 13,511,599 | 4,107,183 | **3.032** | **1.032** | 30.40% |
| `nostage` | `KP=2` | 無 | 2,359,296 | **72.0** | 5,569,223 | 359,111 | **2.361** | **0.361** | 6.45% |
| **`kp2`（production）** | `KP=2` | 有 | 4,456,448 | **136.0** | 10,019,684 | 615,268 | **2.248** | **0.248** | 6.14% |

先行節との整合（別ノード・別ジョブなので桁の一致を見る）: `kp1` の命令
8.110e7 は §20.2 の 81,100,800 と**完全一致**、`kp2` の 6.721e7 は §23.5 と
**完全一致**。コンフリクトは `kp1` 4.107 M 対 §20.2 の 4.057 M（+1.2%）、
`kp2` 6.15e5 対 §23.5 の 6.24e5（−1.4%）。global セクタは 78,643,200 /
95,944,704 で §13.3・§20.2 と**桁まで一致**する。

**まず §20.2 の訂正。** 1 ブロックあたりのストア要求は **72 / 136** であって
80 / 144 ではない。差の 64（ステージングの 8 命令 × 8 ワープ）は正しく、
**両版とも一様に 8 だけ多く数えていた**（`sflux_bnd` を 1.5 本、setup を
概算で入れた部分。実際には setup の 2 本は `tid < 64` / `tid < 112` の
分岐の中にあり、8 ワープのうち一部しか実行しない）。したがって
§20.2 の 2.82 / 2.85 は **3.03 / 3.16** に置き換わり、
**超過は 0.82 ではなく 1.03 wavefront/要求**である。

**そして形には依存しない —— 依存するのは占有率である。** 現行形 `KP=2` の
超過は 0.248 で、旧形の 1.032 の 24% しかない。§20 の測定は旧形のもので、
現行形で測り直したこと自体が次の節の鍵になる。

### 24.4 超過は 2 変数で分離する —— そして片方はアドレスではありえない

上の 4 版は 2×2 になっている。ncu の占有率と global トラフィックを並べる。

| 版 | 達成占有率 | global セクタ/ストア要求 | L1/TEX | **超過** |
|---|---:|---:|---:|---:|
| `kp1_nostage` | 96.4% | 40.67 | 89.31% | **1.164** |
| `kp1` | 96.3% | 17.65 | 91.24% | **1.032** |
| `nostage` | 36.0% | 40.67 | 88.62% | **0.361** |
| `kp2` | 35.9% | 17.65 | 91.78% | **0.248** |

- **占有率を固定して global トラフィックだけを 2.30 倍にすると**
  （ステージングを外す＝`NOSTAGE`）、超過は
  `KP=1` で **+0.132**、`KP=2` で **+0.113** 増える。**ほぼ同じ量**である。
- **global トラフィックを固定して同時ワープ数を 96.3% → 35.9%（×0.373）に
  落とすと**（`KP=1` → `KP=2`）、超過は staged で ×0.240、nostage で ×0.310 に
  落ちる。
- 2 つの効果は**加法的に分離する**。超過 ≈ (占有率で決まる項) + (global
  トラフィックで決まる項) で 4 点が説明できる。

そして `KP=1` → `KP=2` の落ち込みが**アドレス効果ではありえない**ことが示せる。
最大の群である体積フラックスの 6 本は

```
node = tid + t * THR ;  sFluxX[sw_xy(node)] = qn*un ;  ...
sw_xy(idx) = idx ^ (((idx >> 4) & 1) << 2)
sw_z (idx) = idx ^ (((idx >> 6) & 3) << 2)
```

で、`THR` は 256（`KP=1`）でも 128（`KP=2`）でも 32 の倍数だから、
**1 ワープが書く 32 ノードは両形とも同じ 32 連続ノードであり、
`sw_xy` / `sw_z` が折るビット 4 / 6-7 の効き方も両形で同一**である。
つまり**ワープ内のストアアドレスパターンはビット単位で一致している**。
それでも超過は 1.032 → 0.248 に落ちる。**アドレスが同じで計数が 4 分の 1 に
なる以上、超過はアドレスの関数ではない。** これは §20.3 が
「レーン線形に置き換えても減らない」で示したことの、**逆側からの裏づけ**である
（§20.3 はアドレスを変えて計数が動かないことを示し、本節はアドレスを
変えずに計数が動くことを示した）。

### 24.5 機構の言明

以上をまとめると、`l1tex__data_bank_conflicts_pipe_lsu_mem_shared_op_st` が
このカーネルで報告する 30% は、次の 2 つの和である。

- **アドレス由来のバンクコンフリクト。** マイクロベンチが 2 の冪で
  再現するもの。**このカーネルではこれは 0 である** —— 3 群すべての
  レイアウトは設計どおりコンフリクトフリーで、§13.2 の主張も
  §20.3 のレーン線形化が減らさなかったことも、どちらも正しい。
- **L1TEX Data-Stage の競合。** カウンタは「Data-Stage が処理した
  wavefront 数」＝**データ段のサイクル計数**であり、同じ L1TEX が
  global ロードもさばいているとき、shared ストア 1 要求が占めるデータ段の
  サイクルは最小充当を超える。マイクロベンチはこれを**アドレスを固定した
  まま 0 → 0.150 wavefront/要求として単離した**。実カーネルではこの項が
  同時ワープ数と global トラフィック量の両方で動き、超過 1.03 の**全部**を
  占めている。

**§20.5 が「1 変数で切り分ける測定はしていない」と書いた仮説
（LSU の wavefront 再発行）は、これで単離された。** 向きも一致する:
このカーネルは §0 以来 L1/TEX 91% で飽和しており、global ロードと
shared ストアが同じ L1TEX を奪い合う次数である。§23.5 が
「出力タイルはコンフリクトを減らす目的で触っていないのに 85% 消した」と
書いた現象も、これで説明が付く —— **出力タイルが減らしたのは
アドレス衝突ではなく、L1TEX に同時に群がるワープの数**である。

採否は動かない。§20.4 の「天井は負」はそのままで、**この計数は
削りにいく対象ではない**（アドレスを直しても 0 の部分を 0 にするだけで、
残りは L1TEX の混雑そのものだから、減らす手は混雑を減らす手＝
§23 の出力タイルのような手しかない）。

### 24.6 `p31_gap_study.md` §32.6 との関係 —— 別族ではなく、同族で、幅の説明は誤り

§20.3 は「この計数は p=31 y カーネルの st コンフリクト 16〜21% と同じ族で、
**128-bit shared ストアに固有**のもの」と書き、`p31_gap_study.md` §32.6 も
同じ言い方をしている。**幅による説明のほうは、本節のマイクロベンチが
反証する。**

| | wf/要求 | コンフリクト |
|---|---:|---:|
| `d128_linear`（レーン線形 128-bit） | **4.000** | **0** |
| `d64_linear`（レーン線形 64-bit） | 2.000 | 0 |

**レーン線形な 128-bit shared ストアはコンフリクトを 1 も出さない。**
したがって「128-bit ストアに固有で、アドレスの選び方で消せない計数」という
性格づけは、**幅の性質としては成り立たない**。正しくは、p=7 の 64-bit と
p=31 の 128-bit は**同族**であり、共通項は幅ではなく
**L1TEX Data-Stage の競合**である。§32.6 の観測（`sFV` のストアは設計上
コンフリクトフリーで、レーン線形に置き換えても数字が動かない）は、
本節の機構でそのまま説明できる。

**測れていないことを明記する**: 本節は p=31 の y カーネルを測り直していない。
言えるのは「幅による説明は排除された」ことまでで、
**p=31 の残り 16.0% が実際に占有率と global トラフィックで動くかどうかは
未測定**である。p=31 で同じ 2×2（占有率 × global トラフィック）を組めば
確かめられるが、§32.7 が天井 0% を測って閉じている以上、採否は動かない。

### 24.7 まだ測れていないもの（明記）

- **マイクロベンチは機構の存在を示したが、大きさは再現していない。**
  `d64_linear_gpressure` の超過は 0.150 で、実カーネルの 1.03 の 1/7 である。
  global ロード本数・占有率・shared ストア密度を掃引して超過を予測する
  定量モデルは作っていない。**「同符号・同機構である」までが本節の主張で、
  「1.03 のうち何割がどの変数で決まるか」は測っていない。**
- **超過サイクルの正体**（要求の再発行か、データ段の調停スロットか、
  Data-Stage のカウント粒度そのものか）は区別していない。カウンタからは
  「アドレス由来ではない Data-Stage サイクルである」以上のことは決まらない。
- **占有率と global トラフィックのどちらが主かは、2×2 の 4 点でしか見ていない。**
  占有率を連続に振る掃引（`P7_BPSM` を 8→2 で動かして超過を追う）はしていない。
- p=31 y カーネルの再測定（§24.6）。
- shared **ロード**側の wf/要求（`kp1` 2.376、`kp2` 2.052）は 64-bit と
  128-bit の `LDS` が混在するので最小充当が一様でなく、本節では分解していない。

### 24.8 記録

- **カーネル変更はゼロ。** `cuda_dg_kernels_tc.cu` も `fused_kernel_geom.h` も
  1 行も触っていない。`reports/README.md` の p=7 行（`FUSED_TC` 254.15
  µs/stage、§23.4）は**動かない**。数値検証は §23.6 のまま有効である。
- 追加したのは測定用の 2 ファイルだけ:
  **`scripts/micro_shared_wavefront.cu`**（マイクロベンチ本体、11 パターン）と
  **`scripts/build_p7tc_variant.sh`**（`cuda_dg_kernels_tc.cu` だけ再コンパイル
  して凍結実行ファイルを作る）。`jobs/job_p7_shwf.sh` は gitignore なので、
  ncu コマンドは本節冒頭に転記した。
- §20.5 の未解明は**機構つきで閉じた**。§20.2 の要求数 80 / 144 と
  wf/要求 2.82 / 2.85 は**訂正される**（72 / 136、3.03 / 3.16）。
  §20 の結論そのもの（増分は率ではなく要求数、天井は負）は**変わらない**。
