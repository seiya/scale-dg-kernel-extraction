...
### 7.1 追記: 現行 commit（`78fbbf8`）での最速パスの達成効率（2026-08-25）

上の表は `514853f` 時点の値である。その後 §8.4〜§8.8 で volume flux のロード、
lift の epilogue 融合、境界流束の重ね合わせ、RK 更新の 1 次元化が入ったので、
**各次数の最速パスだけ**を採り直した（job 46493、時間は nsys `nstep=20`、
DRAM バイトは ncu 実測、分母は 40.1 TFLOP/s と 7.9 TB/s）。

**p=7, `CUDAFORTRAN_FUSED_TC`, Ne=32³**

| | µs | TFLOP/s | 対 40.1 | TB/s | 対 7.9 |
|---|---:|---:|---:|---:|---:|
| tendency（単一カーネル） | 277.5 | **5.02** | 12.5% | **5.75** | **72.7%** |
| 1 step（GPU カーネル計） | 1072.9 | 3.89 | 9.7% | 5.89 | 74.6% |
| 1 step（wall = Main/1000） | 1131.0 | 3.69 | 9.2% | 5.59 | 70.8% |

**p=255, `CUDAFORTRAN_GEMM_FUSED`, Ne=1**

| | µs | TFLOP/s | 対 40.1 | TB/s | 対 7.9 |
|---|---:|---:|---:|---:|---:|
| tendency（クリティカルパス） | 966.9 | **27.01** | **67.3%** | 2.62 | 33.1% |
| ├ `volume_flux_kernel` | 129.2 | 0.39 | 1.0% | 6.92 | **87.5%** |
| ├ x GEMM `Gemm<64,128,16>` | 254.4 | 33.77 | **84.2%** | 1.45 | 18.4% |
| ├ y GEMM `GemmBatched<64,64,16>` | 243.3 | 35.31 | **88.0%** | 0.93 | 11.8% |
| └ z GEMM + assembly + lift | 340.0 | 26.11 | 65.1% | 2.75 | 34.8% |
| （`elembnd`、2 本目のストリームで隠れる） | 26.6 | 0.30 | 0.8% | 3.98 | 50.3% |
| 1 step（GPU カーネル計） | 3145.4 | 24.91 | 62.1% | 2.91 | 36.8% |
| 1 step（wall = Main/1000） | 3232.0 | 24.24 | 60.4% | 2.83 | 35.8% |

tendency のクリティカルパスは `elembnd_flux_kernel` を含まない（§8.7 で x GEMM の
裏に移した）。1 step は tendency 3 回 + RK 更新（stage 1 が 76.9 µs、stage≥2 が
74.3 µs ×2）+ halo である。

読み取れること:

- **同じ 16.78 M 自由度でも性格が正反対である。** p=7 は FLOP 12.5% / DRAM 73%
  / L1 90.3% で帯域・L1 律速、p=255 は FLOP 67.3% / DRAM 33% で演算律速。
  §10-3 の「同じ DOF 数は同じ GPU 問題を意味しない」がそのまま数字に出ている。
- **p=7 の TC 経路は §7 の 2.86 TFLOP/s から 5.02 TFLOP/s になった**（486.3 →
  277.5 µs/call）。占有率と shared memory レイアウトの作業（`tc_paper_survey`
  §7〜§11）の効果である。
- **p=255 の tendency は 22.62 → 27.01 TFLOP/s（56.4 → 67.3%）。** 67.3% に
  留まっている差分の大半は `volume_flux_kernel` の 129 µs（FLOP 1% だが
  DRAM 87.5% で帯域は使い切っている）と z GEMM の 65.1% で、x/y GEMM を
  88% から上げる余地はほとんど無い。
- **GPU の稼働率は高い。** p=7 で 1072.9 / 1131.0 = 94.9%、p=255 で
  3145.4 / 3232.0 = 97.3% がカーネル実行時間である（§8.2 のストリーム統一と
